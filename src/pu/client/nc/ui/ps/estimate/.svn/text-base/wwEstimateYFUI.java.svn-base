 ...
 *
 */
public class wwEstimateYFUI extends nc.ui.pub.ToftPanel 
...
public void afterEdit(BillEditEvent event) {
	...
	}else if(event.getKey().equals("custname")){
		
		int rw = getBillCardPanel().getBillTable().getSelectedRow();
		getBillCardPanel().getBillModel().execEditFormulasByKey(rw, "cproviderbaseid");
		getBillCardPanel().getBillModel().execEditFormulasByKey(rw, "cprovidermangid");
		getBillCardPanel().getBillModel().execLoadFormulaByKey("custname");
	}
}
/**
* 功能描述：行变换事件处理
...
private BillCardPanel getBillCardPanel() {
	if (m_billPanel == null) {
		try {
			...
			// user code begin {1}
			// 加载模板
			BillData bd = new BillData(m_billPanel.getTempletData("0001AA1000000007BDHH"));
...
private BillData initDecimal(BillData bd) {
	...
	BillItem item6 = bd.getBodyItem("custname");
//	item6.setLock(true);
	item6.setEdit(true);
...
public void onEstimate() {
	...
	if (nSelected == null || nSelected.length == 0) {
		MessageDialog.showErrorDlg(this, "委外运费暂估", "未选中入库单！");
		return;
	}
	...
	for (int i = 0; i < nSelected.length; i++) {
		...
		if (nOutNum == null || nOutNum.doubleValue() == 0.0)
			sMessage.append(vo.getVbillcode() + "入库单的入库数量为零，不能暂估！");
	}
...
//		paraList.add(m_bZGYF);
		...
		for (int i = 0; i < VOs.length; i++) {
			...
			if(nmoney == null || nmoney.equals(0.0)){
				...
				VOs[i].setNmoney(nprice);
				VOs[i].setNtaxprice(ntaxprice);
				VOs[i].setNtotalmoney(ntaxprice);
				int vnum = (Integer) v.get(i);
				String cproviderbaseid =  getBillCardPanel().getBodyValueAt(vnum, "cproviderbaseid").toString();
				String cprovidermangid =  getBillCardPanel().getBodyValueAt(vnum, "cprovidermangid").toString();
				VOs[i].setCproviderbaseid(cproviderbaseid);
				VOs[i].setCprovidermangid(cprovidermangid);
			}
		}
		
		EstimateHelper.estimate_wwyf(VOs,paraList,cl);

		timer.addExecutePhase("数据库更新$$$$操作$$$处理时间#######");

	} catch (java.sql.SQLException e) {
		MessageDialog.showErrorDlg(this, "入库单暂估", "SQL语句错误！");
		SCMEnv.out(e);
		return;
	} catch (ArrayIndexOutOfBoundsException e) {
		MessageDialog.showErrorDlg(this, "入库单暂估", "数组越界错误！");
		SCMEnv.out(e);
		return;
	} catch (NullPointerException e) {
		MessageDialog.showErrorDlg(this, "入库单暂估", "空指针错误！");
		SCMEnv.out(e);
		return;
	} catch (nc.vo.pub.BusinessException e) {
		MessageDialog.showErrorDlg(this, "入库单暂估", e.getMessage());
		SCMEnv.out(e);
		return;
	} catch (Exception e) {
		MessageDialog.showErrorDlg(this, "入库单暂估", e.getMessage());
	...
	if (m_condClient.isCloseOK()) {
		...
		try {
			...
			if (m_sZG.toUpperCase().equals("N")) {
				m_estimate1VOs = EstimateHelper.queryEstimate_wwyf(m_sUnitCode, conditionVO, m_sZG, m_sEstPriceSource);
				...
				m_estimate2VOs = EstimateHelper.queryEstimate_wwyf(m_sUnitCode, conditionVO, m_sZG, m_sEstPriceSource);
				if (m_estimate2VOs == null || m_estimate2VOs.length == 0) {
					MessageDialog.showHintDlg(this, "入库单查询", "没有符合条件的入库单！");
...
public void onUnEstimate() {
	...
	try {
		EstimateHelper.antiEstimate_wwyf(VOs, getClientEnvironment().getUser().getPrimaryKey());
	} catch (java.sql.SQLException e) {
...
private void setPartEditable() {
	...
	for (int i = 0; i < items.length; i++) {
		items[i].setEnabled("nprice".equals(items[i].getKey().trim()) 
				...
				|| "custname".equals(items[i].getKey().trim()));
