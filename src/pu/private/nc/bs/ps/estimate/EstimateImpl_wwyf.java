 ...
 * @author：
 */
public class EstimateImpl_wwyf implements IPuToIc_EstimateImpl,nc.itf.pu.inter.IPuToSc_EstimateBO, IEstimate_wwyf,nc.bs.pub.para.IBeforSaveSysinit{

/**
 * EstimateImpl 构造子注解。
 */
public EstimateImpl_wwyf() {
	super();
}
 ...
 * 暂估应付保存
 */
public void saveBillForARAP_wwyf(wwEstimateVO VOs[], ArrayList paraList) throws BusinessException {
	String sCurrTypeID = (String)paraList.get(0);
	String cOperator = (String)paraList.get(1);
	UFDate dCurrDate = (UFDate)paraList.get(2);
	nc.vo.scm.pu.Timer timer = new nc.vo.scm.pu.Timer();
...
//    DJZBVO arapVO[] = (DJZBVO[]) PfUtilTools.runChangeDataAry("47", "D1", icVOs);
    if(arapVO == null || arapVO.length == 0) return;
		...
		for (int i = 0; i < VOs.length; i++) {
			...
			bodyVO[0].setHbbm(VOs[i].getCproviderbaseid()); //供应商基本ID
			bodyVO[0].setKsbm_cl(VOs[i].getCprovidermangid());//客商管理档案
			bodyVO[0].setCinventoryid(VOs[i].getCbaseid()); //存货基本ID
			...
			bodyVO[0].setPausetransact(new UFBoolean(true));
			bodyVO[0].setOthersysflag("委外运费暂估");
			v2.addElement(bodyVO[0]);
		}
...
private GeneralBillVO switchVOFromEstimate2IC_wwyf(wwEstimateVO estVO){
  ...
  headVO.setCdptid(estVO.getCdptid());
  headVO.setCproviderid(estVO.getCproviderbaseid());
  headVO.setVnote(estVO.getVnote());
  headVO.setCwhsmanagerid(estVO.getCwhsmanagerid());
  headVO.setCbizid(estVO.getCoperatorid());
 ...
 * 修改：晁志平　FOR　V30
 */
public void antiEstimate_wwyf(wwEstimateVO VOs[], String cOperator)
	throws BusinessException {
...
//		}
		...
		if (VOs != null && VOs.length > 0) {
			...
			if(myService0.isEnabled(unitCode, "AP")){
				//应付启用,删除暂估应付
				//******
				deleteBillForZGAP_wwyf(VOs);
				//******
			}
			
			//zhwj 回写
			dmo.backICStatus_wwyf(VOs, status[0], "");
...
//王凯飞
private void deleteBillForZGAP_wwyf(wwEstimateVO VOs[])
throws BusinessException {
	Vector v1 = new Vector();
	for(int i = 0; i < VOs.length; i++){
//		if(VOs[i].getBzgyf().booleanValue()) //zhwj
 ...
 * @exception java.rmi.RemoteException 异常说明。
 */
public wwEstimateVO[] queryEstimate_wwyf(String unitCode, ConditionVO conditionVO[], String sZG, String sEstPriceSource)
	throws BusinessException {
    ...
    if(strDataPowerSql != null) sCondition += " and " + strDataPowerSql;
		...
		wwEstimateDMO dmo = new wwEstimateDMO();
		estimates = dmo.queryEstimate_wwyf(unitCode, sCondition, sZG);
...
/**
 * 功能描述:委外运费暂估      zhwj 2012-08-09
 * 输入参数:VO[],当前操作员ID,当前日期
 * 创建：熊海情
 * 修改：晁志平 FOR  V30
 */
public void estimate_wwyf(wwEstimateVO VOs[], ArrayList paraList,ClientLink cl)
	throws BusinessException {
	nc.vo.scm.pu.Timer timer = new nc.vo.scm.pu.Timer();
	timer.start();
