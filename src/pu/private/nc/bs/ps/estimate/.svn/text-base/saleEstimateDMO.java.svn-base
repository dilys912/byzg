 ...
 * @author：
 */
public class saleEstimateDMO extends DataManageObject {
/**
 * EstimateDMO 构造子注解。
 *
 ...
 */
public saleEstimateDMO() throws javax.naming.NamingException, SystemException {
	super();
}
/**
 * EstimateDMO 构造子注解。
 ...
 * @exception nc.bs.pub.SystemException 父类构造子抛出的异常。
 */
public saleEstimateDMO(String dbName) throws javax.naming.NamingException, SystemException {
	super(dbName);
}
 ...
 * 根据订单行ID查询订单税率和扣税类别
 */
public Hashtable queryOrderTaxRate_sale(String sBodyKey[]) throws SQLException{
	String sql =
		"select corder_bid, ntaxrate from so_saleorder_b where dr = 0 and corder_bid in ";
...
public void backICStatus_so(saleEstimateVO[] vOs,String status,String date) throws BusinessException{
	BaseDAO basdao = new BaseDAO();
	if(vOs!=null&&vOs.length>0){
		for(int i=0;i<vOs.length;i++){
			String cgeneralbid = vOs[i].getCgeneralbid()==null?"":vOs[i].getCgeneralbid().toString();
 ...
 * 作者：王凯飞
 */
public saleEstimateVO[] queryEstimate_saleyf(String unitCode, String sCondition, String sZG) throws SQLException {

	/*************************************************************/
	// 保留的系统管理接口：
	beforeCallMethod("nc.bs.ps.estimate.EstimateDMO", "queryEstimate_saleyf", new Object[] { unitCode, sCondition, sZG });
	/*************************************************************/
	...
	.append("        vbillcode, ") 
	.append("        dbilldate, ") 
	.append("        ccustomerid, ") //ctrancustidccustomerid
	...
	.append("        cbiztype, ") 
	.append("        vbatchcode, ") 
	.append("        csourcebillhid, ") 
	.append("        csourcebillbid, ") 
	.append("        B.crowno, ") 
	.append("        A.pk_calbody, ")
	.append("        A.cwastewarehouseid pktrancust, ");//承运商

	if (sZG.toUpperCase().equals("Y")) {
		sql.append("nvl(man.def20,0) npprice, nvl(noutnum,0)*nvl(man.def20,0) npmoney, dzgdate, ");
	} else {
		sql.append("nvl(man.def20,0) nprice, nvl(noutnum,0)*nvl(man.def20,0) nmny, dzgdate, ");
	}
	...
	.append("        pk_purcorp, ") 
	.append("        vfirstbillcode, ") 
	.append("        iscalculatedinvcost, ") 
	.append("        pk_invmandoc ") ;


	sql.append(" from ic_general_h a,ic_general_b b,ic_general_bb3 c,bd_stordoc     s,bd_busitype    bt,bd_invmandoc   man,so_saleorder_b   sob ");//, dm_delivbill_h dmh
	
	sql.append(" where nvl(a.dr, 0) = 0 ") 
	.append("    and nvl(b.dr, 0) = 0 ") 
	.append("    and a.cbiztype = bt.pk_busitype ") 
	.append("    and a.cwarehouseid = s.pk_stordoc ") 
	.append("    and a.cgeneralhid = b.cgeneralhid ") 
	.append("    and a.cgeneralhid = c.cgeneralhid ") 
	.append("    and b.cgeneralbid = c.cgeneralbid ") 
	.append("    and b.cinventoryid = man.pk_invmandoc ") 
	.append("    and b.cfirstbillbid = sob.corder_bid ") 
//	.append("    and dmh.pk_delivbill_h = b.csourcebillhid ") 
	.append("    and a.cbilltypecode in ('4C') ") 
	.append("    and a.vuserdef16 = '是' ") //制盖在销售订单中自定义的自定义项16为是否送货
	.append("    and (a.fbillflag = 3 OR a.fbillflag = 4) ") 
	.append("    and b.noutnum > 0.0 ") 
	.append("    and a.cwastewarehouseid is not null  ") //承运商不可为空
	.append("    and nvl(b.vuserdef5, 0) < 1 ") 
	.append("    and nvl(a.vuserdef20, 0) <> 1 ") ;
...
//		sql += "and nvl(B.vuserdef19,'未暂估且未结算') in ('已暂估且未结算','已暂估部分结算') ";
	...
	if (sZG.toUpperCase().equals("N"))
//		sql += "and naccountnum1 = 0.0 and abs(ninnum) > 0.0 ";//zhwj 系统结算 回写naccountnum1，则不能暂估或反暂估，当运费暂估通过nvl(B.vuserdef19,'未暂估且未结算') in ('已暂估且未结算','已暂估部分结算')反判断
		sql.append("and  abs(noutnum) > 0.0 ");
...
//		sql += "and naccountnum1 = 0.0 ";//zhwj 系统结算 回写naccountnum1，
	...
	//按入库单号 + 行号排序列
	sql.append("order by vbillcode, crowno ");
	...
	try {
		...
		Object oTmp = null;
		int iIdx = 0;
		saleEstimateVO estimate = null;
		while (rs.next()) {
			iIdx = 1;
			estimate = new saleEstimateVO();
			// A.vuserdef1-10
...
//			estimate.setCfirsttype(rs.getString(iIdx++));
			...
			//B.cfirstbillbid,
			estimate.setCbaseid(rs.getString("cinventoryid"));//存货编码
//			estimate.setCfirstbillbid(rs.getString(iIdx++));
...
//			estimate.setCfirstbillhid(rs.getString(iIdx++));
			...
			//B.ninassistnum,
			oTmp = rs.getObject("noutassistnum");
			estimate.setNoutassistnum(oTmp == null ? new UFDouble(0.0) : new UFDouble(oTmp.toString()));
			//B.hsl,
			...
			estimate.setDbilldate(strTmp == null ? null : new UFDate(strTmp.trim(), false));
			// ccustomerid :
			strTmp = rs.getString("pktrancust");
			estimate.setCcustomerid(strTmp == null ? null : strTmp.trim());
			// cgeneralhid :
...
//			estimate.setCmangid(strTmp == null ? null : strTmp.trim());
			...
			estimate.setVfree5(strTmp == null ? null : strTmp.trim());
			// noutnum :
			oTmp = rs.getObject("noutnum");
			estimate.setNoutnum(oTmp == null ? new UFDouble(0.0) : new UFDouble(oTmp.toString()));
			// nsettlenum :
	...
	if (v.size() > 0) {
		estimates = new saleEstimateVO[v.size()];
		for (int i = 0; i < v.size(); i++)
			estimates[i] = new saleEstimateVO();
		v.copyInto(estimates);
	}
