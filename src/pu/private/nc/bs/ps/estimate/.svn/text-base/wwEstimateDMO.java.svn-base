 ...
 * @author：
 */
public class wwEstimateDMO extends DataManageObject {
/**
 * EstimateDMO 构造子注解。
 *
 ...
 */
public wwEstimateDMO() throws javax.naming.NamingException, SystemException {
	super();
}
/**
 * EstimateDMO 构造子注解。
 ...
 * @exception nc.bs.pub.SystemException 父类构造子抛出的异常。
 */
public wwEstimateDMO(String dbName) throws javax.naming.NamingException, SystemException {
	super(dbName);
}
...
public void backICStatus_ww(wwEstimateVO[] vOs,String status,String date) throws BusinessException{
	BaseDAO basdao = new BaseDAO();
	if(vOs!=null&&vOs.length>0){
		for(int i=0;i<vOs.length;i++){
			String cgeneralbid = vOs[i].getCgeneralbid()==null?"":vOs[i].getCgeneralbid().toString();
 ...
 * 作者：王凯飞
 */
public wwEstimateVO[] queryEstimate_wwyf(String unitCode, String sCondition, String sZG) throws SQLException {
	...
	if (sZG.toUpperCase().equals("Y")) {
		sql.append("nvl(man.def20,0) npprice, nvl(ninnum,0)*nvl(man.def20,0) npmoney, dzgdate, ");
	} else {
		sql.append("nvl(man.def20,0) nprice, nvl(ninnum,0)*nvl(man.def20,0) nmny, dzgdate, ");
	}
	...
	.append("        pk_invmandoc, ") 
	.append("        cbaseid, ") 
	.append("        idiscounttaxtype ") ;



	sql.append(" from ic_general_h a,ic_general_b b,ic_general_bb3 c,bd_stordoc     s,bd_busitype    bt,bd_invmandoc   man,sc_order_b      scb ");
	...
	.append("    AND (fbillflag = 3 OR fbillflag = 4) ") 
	.append("        and BT.verifyrule not in ('V', 'N') ") 
	.append("        and abs(ninnum) > 0.0 ") ;
