 ...
 * @author：
 */
public class EstimateImpl_saleyf implements IPuToIc_EstimateImpl,nc.itf.pu.inter.IPuToSc_EstimateBO, IEstimate_saleyf,nc.bs.pub.para.IBeforSaveSysinit{

/**
 * EstimateImpl 构造子注解。
 */
public EstimateImpl_saleyf() {
	super();
}
...
throws BusinessException {
	...
	saleEstimateVO VO[] = new saleEstimateVO[v1.size()];
	v1.copyInto(VO);
...
throws BusinessException {
    if(!(voaVO instanceof saleEstimateVO[])){
    ...
    saleEstimateVO[] VOs = (saleEstimateVO[]) voaVO;
	nc.vo.scm.pu.Timer timerDebug = new nc.vo.scm.pu.Timer();
	timerDebug.start();
...
public void saveBillForARAP(EstimateVO[] vOs, ArrayList paraList) throws BusinessException {
	...
	try {
		nc.bs.ps.estimate.saleEstimateDMO dmo = new nc.bs.ps.estimate.saleEstimateDMO();
	...
	for (int i = 0; i < VOs.length; i++) {
		...
		if (bodyVO != null && bodyVO.length > 0) {
			for (int j = 0; j < bodyVO.length; j++) {
				...
				//estimateVO
				saleEstimateVO estVO = new saleEstimateVO();
				...
				estVO.setPk_invmandoc(bodyVO[j].getCinventoryid());
...
//				estVO.setCproviderbaseid()???
				...
				estVO.setNoutnum(bodyVO[j].getNoutnum());
	...
	saleEstimateVO estimates[] = new saleEstimateVO[v3.size()];
...
private EstimateVO[] queryBillInfoForIA(EstimateVO[] vOs1) throws BusinessException{
	try{
		//查询存货表体VO所需的数据
		Vector v = queryStockBodyForIA(vOs1);
		if (v == null || v.size() == 0) {
		...
		for (int i = 0; i < vOs1.length; i++) {
			...
			vOs1[i].setVfree1(itemVO.getVfree1());
			vOs1[i].setVfree2(itemVO.getVfree2());
			vOs1[i].setVfree3(itemVO.getVfree3());
			vOs1[i].setVfree4(itemVO.getVfree4());
			vOs1[i].setVfree5(itemVO.getVfree5());
		
			vOs1[i].setVuserdef1(itemVO.getVuserdef1());
			vOs1[i].setVuserdef2(itemVO.getVuserdef2());
			vOs1[i].setVuserdef3(itemVO.getVuserdef3());
			vOs1[i].setVuserdef4(itemVO.getVuserdef4());
			vOs1[i].setVuserdef5(itemVO.getVuserdef5());
			vOs1[i].setVuserdef6(itemVO.getVuserdef6());
			vOs1[i].setVuserdef7(itemVO.getVuserdef7());
			vOs1[i].setVuserdef8(itemVO.getVuserdef8());
			vOs1[i].setVuserdef9(itemVO.getVuserdef9());
			vOs1[i].setVuserdef10(itemVO.getVuserdef10());
	
			vOs1[i].setVuserdef11(itemVO.getVuserdef11());
			vOs1[i].setVuserdef12(itemVO.getVuserdef12());
			vOs1[i].setVuserdef13(itemVO.getVuserdef13());
			vOs1[i].setVuserdef14(itemVO.getVuserdef14());
			vOs1[i].setVuserdef15(itemVO.getVuserdef15());
			vOs1[i].setVuserdef16(itemVO.getVuserdef16());
			vOs1[i].setVuserdef17(itemVO.getVuserdef17());
			vOs1[i].setVuserdef18(itemVO.getVuserdef18());
			vOs1[i].setVuserdef19(itemVO.getVuserdef19());
			vOs1[i].setVuserdef20(itemVO.getVuserdef20());
			
			vOs1[i].setPk_defdocb1(itemVO.getPk_defdoc1());
			vOs1[i].setPk_defdocb2(itemVO.getPk_defdoc2());
			vOs1[i].setPk_defdocb3(itemVO.getPk_defdoc3());
			vOs1[i].setPk_defdocb4(itemVO.getPk_defdoc4());
			vOs1[i].setPk_defdocb5(itemVO.getPk_defdoc5());
			vOs1[i].setPk_defdocb6(itemVO.getPk_defdoc6());
			vOs1[i].setPk_defdocb7(itemVO.getPk_defdoc7());
			vOs1[i].setPk_defdocb8(itemVO.getPk_defdoc8());
			vOs1[i].setPk_defdocb9(itemVO.getPk_defdoc9());
			vOs1[i].setPk_defdocb10(itemVO.getPk_defdoc10());
	
			vOs1[i].setPk_defdocb11(itemVO.getPk_defdoc11());
			vOs1[i].setPk_defdocb12(itemVO.getPk_defdoc12());
			vOs1[i].setPk_defdocb13(itemVO.getPk_defdoc13());
			vOs1[i].setPk_defdocb14(itemVO.getPk_defdoc14());
			vOs1[i].setPk_defdocb15(itemVO.getPk_defdoc15());
			vOs1[i].setPk_defdocb16(itemVO.getPk_defdoc16());
			vOs1[i].setPk_defdocb17(itemVO.getPk_defdoc17());
			vOs1[i].setPk_defdocb18(itemVO.getPk_defdoc18());
			vOs1[i].setPk_defdocb19(itemVO.getPk_defdoc19());
			vOs1[i].setPk_defdocb20(itemVO.getPk_defdoc20());
			
			vOs1[i].setCprojectid(itemVO.getCprojectid());
			vOs1[i].setCprojectphaseid(itemVO.getCprojectphaseid());
			
			vOs1[i].setCvendorid(itemVO.getCvendorid());
		}
	
		//查询存货表头VO所需的数据
		String sGeneralhid[] = new String[vOs1.length];
		String sWarehouseid[] = new String[vOs1.length];
		for(int i = 0; i < vOs1.length; i++){
			sGeneralhid[i] = vOs1[i].getCgeneralhid();
			sWarehouseid[i] = vOs1[i].getCwarehouseid();
		}
		...
		for (int i = 0; i < vOs1.length; i++) {
			...
			vOs1[i].setVuserdefh1(hVO.getVuserdef1());
			vOs1[i].setVuserdefh2(hVO.getVuserdef2());
			vOs1[i].setVuserdefh3(hVO.getVuserdef3());
			vOs1[i].setVuserdefh4(hVO.getVuserdef4());
			vOs1[i].setVuserdefh5(hVO.getVuserdef5());
			vOs1[i].setVuserdefh6(hVO.getVuserdef6());
			vOs1[i].setVuserdefh7(hVO.getVuserdef7());
			vOs1[i].setVuserdefh8(hVO.getVuserdef8());
			vOs1[i].setVuserdefh9(hVO.getVuserdef9());
			vOs1[i].setVuserdefh10(hVO.getVuserdef10());
			
			vOs1[i].setVuserdefh11(hVO.getVuserdef11());
			vOs1[i].setVuserdefh12(hVO.getVuserdef12());
			vOs1[i].setVuserdefh13(hVO.getVuserdef13());
			vOs1[i].setVuserdefh14(hVO.getVuserdef14());
			vOs1[i].setVuserdefh15(hVO.getVuserdef15());
			vOs1[i].setVuserdefh16(hVO.getVuserdef16());
			vOs1[i].setVuserdefh17(hVO.getVuserdef17());
			vOs1[i].setVuserdefh18(hVO.getVuserdef18());
			vOs1[i].setVuserdefh19(hVO.getVuserdef19());
			vOs1[i].setVuserdefh20(hVO.getVuserdef20());
			
			vOs1[i].setPk_defdoch1(hVO.getPk_defdoc1());
			vOs1[i].setPk_defdoch2(hVO.getPk_defdoc2());
			vOs1[i].setPk_defdoch3(hVO.getPk_defdoc3());
			vOs1[i].setPk_defdoch4(hVO.getPk_defdoc4());
			vOs1[i].setPk_defdoch5(hVO.getPk_defdoc5());
			vOs1[i].setPk_defdoch6(hVO.getPk_defdoc6());
			vOs1[i].setPk_defdoch7(hVO.getPk_defdoc7());
			vOs1[i].setPk_defdoch8(hVO.getPk_defdoc8());
			vOs1[i].setPk_defdoch9(hVO.getPk_defdoc9());
			vOs1[i].setPk_defdoch10(hVO.getPk_defdoc10());
	
			vOs1[i].setPk_defdoch11(hVO.getPk_defdoc11());
			vOs1[i].setPk_defdoch12(hVO.getPk_defdoc12());
			vOs1[i].setPk_defdoch13(hVO.getPk_defdoc13());
			vOs1[i].setPk_defdoch14(hVO.getPk_defdoc14());
			vOs1[i].setPk_defdoch15(hVO.getPk_defdoc15());
			vOs1[i].setPk_defdoch16(hVO.getPk_defdoc16());
			vOs1[i].setPk_defdoch17(hVO.getPk_defdoc17());
			vOs1[i].setPk_defdoch18(hVO.getPk_defdoc18());
			vOs1[i].setPk_defdoch19(hVO.getPk_defdoc19());
			vOs1[i].setPk_defdoch20(hVO.getPk_defdoc20());	
			
			vOs1[i].setCdispatcherid(hVO.getCdispatcherid());
      vOs1[i].setBcalculatecost(hVO.getBcalculatecost());
		}
	} catch (Exception e) {
		/*调用采购公用方法按规范抛出异常*/
		nc.bs.pu.pub.PubDMO.throwBusinessException(e);
 ...
 * @param vOs nc.vo.ps.estimate.EstimateVO[]
 */
private String[] getEstimateLockKeys_sale(saleEstimateVO[] vOs) {
	if(vOs==null || vOs.length==0) return null;
 ...
 * @param vOs nc.vo.ps.estimate.EstimateVO[]
 */
private String[] getsaleEstimateLockKeys(saleEstimateVO[] vOs) {
	if(vOs==null || vOs.length==0) return null;
 ...
 * 作者:王凯飞
 */
private String isTimeStampChanged_so(saleEstimateVO[] vOs) throws BusinessException {
 ...
 * 修改:晁志平 FOR  V30 修正BUG：未查询到新时间戳时未报错
 */
private String isTimeStampChanged_sale(saleEstimateVO[] vOs) throws BusinessException {
...
public EstimateVO[] queryEstimate(String unitCode, ConditionVO conditionVO[], String sZG, String sEstPriceSource)
    ...
    if(strDataPowerSql != null) sCondition += " and " + strDataPowerSql;
		...
		if(estimates != null && estimates.length > 0) {
			...
			Vector vTemp = new Vector();
			for(int i = 0; i < estimates.length; i++){
				if(estimates[i].getCfirsttype() != null && estimates[i].getCfirsttype().equals(ScmConst.SO_Order)){//PO_Order
 ...
 */
private Vector queryStockBodyForIA(EstimateVO[] vOs1)
	throws BusinessException {
	Vector v = new Vector();
	try {
		EstimateDMO dmo = new EstimateDMO();
		...
		for (int i = 0; i < vOs1.length; i++) {
			v1.addElement(vOs1[i].getCmangid());
			v2.addElement(vOs1[i].getCwarehouseid());
			v3.addElement(vOs1[i].getCgeneralbid());
		}
 ...
 * 日期:2003/05/29
 * @return nc.vo.ps.settle.StockVO[]
 * @param vOs2 nc.vo.ps.estimate.EstimateVO[]
 */
private StockVO[] switchStockVO(EstimateVO[] vOs2) {
	Vector v = new Vector();
	...
	for(int i = 0; i < vOs2.length; i++){
		...
		vo.setCprovidermangid(vOs2[i].getCprovidermangid());
		vo.setCproviderbaseid(vOs2[i].getCproviderbaseid());
		vo.setPk_corp(vOs2[i].getPk_corp());
		vo.setVbillcode(vOs2[i].getVbillcode());
		vo.setCgeneralhid(vOs2[i].getCgeneralhid());
		vo.setCgeneralbid(vOs2[i].getCgeneralbid());
		vo.setCgeneralbb3(vOs2[i].getCgeneralbb3());
		vo.setCmangid(vOs2[i].getCmangid());
		vo.setCbaseid(vOs2[i].getCbaseid());

		//数量：入库数量
		vo.setNinnum(vOs2[i].getNinnum());
		//单价：暂估单价
		vo.setNprice(vOs2[i].getNprice());
		//金额：暂估金额
		vo.setNmoney(vOs2[i].getNmoney());
		//累计结算数量
		vo.setNaccumsettlenum(vOs2[i].getNsettlenum());
		//累计结算金额
		vo.setNaccumsettlemny(vOs2[i].getNsettlemny());
		...
		//来源信息
		vo.setCsourcebillhid(vOs2[i].getCsourcebillhid());
		vo.setCsourcebillbid(vOs2[i].getCsourcebillbid());
		vo.setVsourcebillcode(null);
		//源头信息
		vo.setCfirstbillhid(vOs2[i].getCfirstbillhid());
		vo.setCfirstbillbid(vOs2[i].getCfirstbillbid());
		
		vo.setVuserdefh1(vOs2[i].getVuserdefh1());
		vo.setVuserdefh2(vOs2[i].getVuserdefh2());
		vo.setVuserdefh3(vOs2[i].getVuserdefh3());
		vo.setVuserdefh4(vOs2[i].getVuserdefh4());
		vo.setVuserdefh5(vOs2[i].getVuserdefh5());
		vo.setVuserdefh6(vOs2[i].getVuserdefh6());
		vo.setVuserdefh7(vOs2[i].getVuserdefh7());
		vo.setVuserdefh8(vOs2[i].getVuserdefh8());
		vo.setVuserdefh9(vOs2[i].getVuserdefh9());
		vo.setVuserdefh10(vOs2[i].getVuserdefh10());

		vo.setVuserdefh11(vOs2[i].getVuserdefh11());
		vo.setVuserdefh12(vOs2[i].getVuserdefh12());
		vo.setVuserdefh13(vOs2[i].getVuserdefh13());
		vo.setVuserdefh14(vOs2[i].getVuserdefh14());
		vo.setVuserdefh15(vOs2[i].getVuserdefh15());
		vo.setVuserdefh16(vOs2[i].getVuserdefh16());
		vo.setVuserdefh17(vOs2[i].getVuserdefh17());
		vo.setVuserdefh18(vOs2[i].getVuserdefh18());
		vo.setVuserdefh19(vOs2[i].getVuserdefh19());
		vo.setVuserdefh20(vOs2[i].getVuserdefh20());
		
		vo.setPk_defdoch1(vOs2[i].getPk_defdoch1());
		vo.setPk_defdoch2(vOs2[i].getPk_defdoch2());
		vo.setPk_defdoch3(vOs2[i].getPk_defdoch3());
		vo.setPk_defdoch4(vOs2[i].getPk_defdoch4());
		vo.setPk_defdoch5(vOs2[i].getPk_defdoch5());
		vo.setPk_defdoch6(vOs2[i].getPk_defdoch6());
		vo.setPk_defdoch7(vOs2[i].getPk_defdoch7());
		vo.setPk_defdoch8(vOs2[i].getPk_defdoch8());
		vo.setPk_defdoch9(vOs2[i].getPk_defdoch9());
		vo.setPk_defdoch10(vOs2[i].getPk_defdoch10());

		vo.setPk_defdoch11(vOs2[i].getPk_defdoch11());
		vo.setPk_defdoch12(vOs2[i].getPk_defdoch12());
		vo.setPk_defdoch13(vOs2[i].getPk_defdoch13());
		vo.setPk_defdoch14(vOs2[i].getPk_defdoch14());
		vo.setPk_defdoch15(vOs2[i].getPk_defdoch15());
		vo.setPk_defdoch16(vOs2[i].getPk_defdoch16());
		vo.setPk_defdoch17(vOs2[i].getPk_defdoch17());
		vo.setPk_defdoch18(vOs2[i].getPk_defdoch18());
		vo.setPk_defdoch19(vOs2[i].getPk_defdoch19());
		vo.setPk_defdoch20(vOs2[i].getPk_defdoch20());

		vo.setVuserdefb1(vOs2[i].getVuserdef1());
		vo.setVuserdefb2(vOs2[i].getVuserdef2());
		vo.setVuserdefb3(vOs2[i].getVuserdef3());
		vo.setVuserdefb4(vOs2[i].getVuserdef4());
		vo.setVuserdefb5(vOs2[i].getVuserdef5());
		vo.setVuserdefb6(vOs2[i].getVuserdef6());
		vo.setVuserdefb7(vOs2[i].getVuserdef7());
		vo.setVuserdefb8(vOs2[i].getVuserdef8());
		vo.setVuserdefb9(vOs2[i].getVuserdef9());
		vo.setVuserdefb10(vOs2[i].getVuserdef10());

		vo.setVuserdefb11(vOs2[i].getVuserdef11());
		vo.setVuserdefb12(vOs2[i].getVuserdef12());
		vo.setVuserdefb13(vOs2[i].getVuserdef13());
		vo.setVuserdefb14(vOs2[i].getVuserdef14());
		vo.setVuserdefb15(vOs2[i].getVuserdef15());
		vo.setVuserdefb16(vOs2[i].getVuserdef16());
		vo.setVuserdefb17(vOs2[i].getVuserdef17());
		vo.setVuserdefb18(vOs2[i].getVuserdef18());
		vo.setVuserdefb19(vOs2[i].getVuserdef19());
		vo.setVuserdefb20(vOs2[i].getVuserdef20());

		vo.setPk_defdocb1(vOs2[i].getPk_defdocb1());
		vo.setPk_defdocb2(vOs2[i].getPk_defdocb2());
		vo.setPk_defdocb3(vOs2[i].getPk_defdocb3());
		vo.setPk_defdocb4(vOs2[i].getPk_defdocb4());
		vo.setPk_defdocb5(vOs2[i].getPk_defdocb5());
		vo.setPk_defdocb6(vOs2[i].getPk_defdocb6());
		vo.setPk_defdocb7(vOs2[i].getPk_defdocb7());
		vo.setPk_defdocb8(vOs2[i].getPk_defdocb8());
		vo.setPk_defdocb9(vOs2[i].getPk_defdocb9());
		vo.setPk_defdocb10(vOs2[i].getPk_defdocb10());

		vo.setPk_defdocb11(vOs2[i].getPk_defdocb11());
		vo.setPk_defdocb12(vOs2[i].getPk_defdocb12());
		vo.setPk_defdocb13(vOs2[i].getPk_defdocb13());
		vo.setPk_defdocb14(vOs2[i].getPk_defdocb14());
		vo.setPk_defdocb15(vOs2[i].getPk_defdocb15());
		vo.setPk_defdocb16(vOs2[i].getPk_defdocb16());
		vo.setPk_defdocb17(vOs2[i].getPk_defdocb17());
		vo.setPk_defdocb18(vOs2[i].getPk_defdocb18());
		vo.setPk_defdocb19(vOs2[i].getPk_defdocb19());
		vo.setPk_defdocb20(vOs2[i].getPk_defdocb20());
		
		if(vo.getCsourcebillhid() != null && vo.getCsourcebillhid().trim().length() > 0
				&& (vOs2[i].getBzgyf() == null || !vOs2[i].getBzgyf().booleanValue())) v.addElement(vo);
	}
 ...
 * 2007-03-19 xhq
 */
private GeneralBillVO switchVOFromEstimate2IC(EstimateVO vOs){
  GeneralBillItemVO bodyVO = new GeneralBillItemVO();
  
  bodyVO.setCgeneralbid(vOs.getCgeneralbid());
  bodyVO.setCgeneralhid(vOs.getCgeneralhid());

  bodyVO.setCinventoryid(vOs.getCmangid());
  bodyVO.setVbatchcode(vOs.getVbatchcode());
  bodyVO.setNinnum(vOs.getNinnum());  
  bodyVO.setNmny(vOs.getNmoney());
  bodyVO.setNprice(vOs.getNprice());

  bodyVO.setCaccountunitid(vOs.getCastunitid());
  bodyVO.setHsl(vOs.getHsl());
  bodyVO.setNinassistnum(vOs.getNinassistnum());
  
  bodyVO.setCfirstbillhid(vOs.getCfirstbillhid());
  bodyVO.setCfirstbillbid(vOs.getCfirstbillbid());
  bodyVO.setCfirsttype(vOs.getCfirsttype());

  bodyVO.setVfree1(vOs.getVfree1());
  bodyVO.setVfree2(vOs.getVfree2());
  bodyVO.setVfree3(vOs.getVfree3());
  bodyVO.setVfree4(vOs.getVfree4());
  bodyVO.setVfree5(vOs.getVfree5());

  bodyVO.setCprojectid(vOs.getCprojectid());
  bodyVO.setCprojectphaseid(vOs.getCprojectphaseid());

  bodyVO.setVuserdef1(vOs.getVuserdef1());
  bodyVO.setVuserdef2(vOs.getVuserdef2());
  bodyVO.setVuserdef3(vOs.getVuserdef3());
  bodyVO.setVuserdef4(vOs.getVuserdef4());
  bodyVO.setVuserdef5(vOs.getVuserdef5());
  bodyVO.setVuserdef6(vOs.getVuserdef6());
  bodyVO.setVuserdef7(vOs.getVuserdef7());
  bodyVO.setVuserdef8(vOs.getVuserdef8());
  bodyVO.setVuserdef9(vOs.getVuserdef9());
  bodyVO.setVuserdef10(vOs.getVuserdef10());
    
  bodyVO.setAttributeValue("vuserdef11", vOs.getVuserdef11());
  bodyVO.setAttributeValue("vuserdef12", vOs.getVuserdef12());
  bodyVO.setAttributeValue("vuserdef13", vOs.getVuserdef13());
  bodyVO.setAttributeValue("vuserdef14", vOs.getVuserdef14());
  bodyVO.setAttributeValue("vuserdef15", vOs.getVuserdef15());
  bodyVO.setAttributeValue("vuserdef16", vOs.getVuserdef16());
  bodyVO.setAttributeValue("vuserdef17", vOs.getVuserdef17());
  bodyVO.setAttributeValue("vuserdef18", vOs.getVuserdef18());
  bodyVO.setAttributeValue("vuserdef19", vOs.getVuserdef19());
  bodyVO.setAttributeValue("vuserdef20", vOs.getVuserdef20());
    
  bodyVO.setAttributeValue("pk_defdoc1", vOs.getPk_defdocb1());
  bodyVO.setAttributeValue("pk_defdoc2", vOs.getPk_defdocb2());
  bodyVO.setAttributeValue("pk_defdoc3", vOs.getPk_defdocb3());
  bodyVO.setAttributeValue("pk_defdoc4", vOs.getPk_defdocb4());
  bodyVO.setAttributeValue("pk_defdoc5", vOs.getPk_defdocb5());
  bodyVO.setAttributeValue("pk_defdoc6", vOs.getPk_defdocb6());
  bodyVO.setAttributeValue("pk_defdoc7", vOs.getPk_defdocb7());
  bodyVO.setAttributeValue("pk_defdoc8", vOs.getPk_defdocb8());
  bodyVO.setAttributeValue("pk_defdoc9", vOs.getPk_defdocb9());
  bodyVO.setAttributeValue("pk_defdoc10", vOs.getPk_defdocb10());

  bodyVO.setAttributeValue("pk_defdoc11", vOs.getPk_defdocb11());
  bodyVO.setAttributeValue("pk_defdoc12", vOs.getPk_defdocb12());
  bodyVO.setAttributeValue("pk_defdoc13", vOs.getPk_defdocb13());
  bodyVO.setAttributeValue("pk_defdoc14", vOs.getPk_defdocb14());
  bodyVO.setAttributeValue("pk_defdoc15", vOs.getPk_defdocb15());
  bodyVO.setAttributeValue("pk_defdoc16", vOs.getPk_defdocb16());
  bodyVO.setAttributeValue("pk_defdoc17", vOs.getPk_defdocb17());
  bodyVO.setAttributeValue("pk_defdoc18", vOs.getPk_defdocb18());
  bodyVO.setAttributeValue("pk_defdoc19", vOs.getPk_defdocb19());
  bodyVO.setAttributeValue("pk_defdoc20", vOs.getPk_defdocb20());
    
  bodyVO.setDbizdate(vOs.getDbizdate());
  bodyVO.setCvendorid(vOs.getCvendorid());

  GeneralBillHeaderVO headVO = new GeneralBillHeaderVO();
  headVO.setPk_corp(vOs.getPk_corp());
  headVO.setCbilltypecode(vOs.getCbilltypecode());
  headVO.setCgeneralhid(vOs.getCgeneralhid());

  headVO.setCdispatcherid(vOs.getCdispatcherid());

  headVO.setCbiztypeid(vOs.getCbiztype());
  headVO.setPk_calbody(vOs.getCcalbodyid());
  headVO.setCwarehouseid(vOs.getCwarehouseid());

  headVO.setCdptid(vOs.getCdptid());
  headVO.setCproviderid(vOs.getCprovidermangid());

  headVO.setVnote(vOs.getVnote());
  headVO.setCwhsmanagerid(vOs.getCwhsmanagerid());
  headVO.setCbizid(vOs.getCoperatorid());

  headVO.setVbillcode(vOs.getVbillcode());
  headVO.setIscalculatedinvcost(vOs.getBcalculatecost());

  headVO.setVuserdef1(vOs.getVuserdefh1());
  headVO.setVuserdef2(vOs.getVuserdefh2());
  headVO.setVuserdef3(vOs.getVuserdefh3());
  headVO.setVuserdef4(vOs.getVuserdefh4());
  headVO.setVuserdef5(vOs.getVuserdefh5());
  headVO.setVuserdef6(vOs.getVuserdefh6());
  headVO.setVuserdef7(vOs.getVuserdefh7());
  headVO.setVuserdef8(vOs.getVuserdefh8());
  headVO.setVuserdef9(vOs.getVuserdefh9());
  headVO.setVuserdef10(vOs.getVuserdefh10());
    
  headVO.setAttributeValue("vuserdef11", vOs.getVuserdefh11());
  headVO.setAttributeValue("vuserdef12", vOs.getVuserdefh12());
  headVO.setAttributeValue("vuserdef13", vOs.getVuserdefh13());
  headVO.setAttributeValue("vuserdef14", vOs.getVuserdefh14());
  headVO.setAttributeValue("vuserdef15", vOs.getVuserdefh15());
  headVO.setAttributeValue("vuserdef16", vOs.getVuserdefh16());
  headVO.setAttributeValue("vuserdef17", vOs.getVuserdefh17());
  headVO.setAttributeValue("vuserdef18", vOs.getVuserdefh18());
  headVO.setAttributeValue("vuserdef19", vOs.getVuserdefh19());
  headVO.setAttributeValue("vuserdef20", vOs.getVuserdefh20());
    
  headVO.setAttributeValue("pk_defdoc1", vOs.getPk_defdoch1());
  headVO.setAttributeValue("pk_defdoc2", vOs.getPk_defdoch2());
  headVO.setAttributeValue("pk_defdoc3", vOs.getPk_defdoch3());
  headVO.setAttributeValue("pk_defdoc4", vOs.getPk_defdoch4());
  headVO.setAttributeValue("pk_defdoc5", vOs.getPk_defdoch5());
  headVO.setAttributeValue("pk_defdoc6", vOs.getPk_defdoch6());
  headVO.setAttributeValue("pk_defdoc7", vOs.getPk_defdoch7());
  headVO.setAttributeValue("pk_defdoc8", vOs.getPk_defdoch8());
  headVO.setAttributeValue("pk_defdoc9", vOs.getPk_defdoch9());
  headVO.setAttributeValue("pk_defdoc10", vOs.getPk_defdoch10());

  headVO.setAttributeValue("pk_defdoc11", vOs.getPk_defdoch11());
  headVO.setAttributeValue("pk_defdoc12", vOs.getPk_defdoch12());
  headVO.setAttributeValue("pk_defdoc13", vOs.getPk_defdoch13());
  headVO.setAttributeValue("pk_defdoc14", vOs.getPk_defdoch14());
  headVO.setAttributeValue("pk_defdoc15", vOs.getPk_defdoch15());
  headVO.setAttributeValue("pk_defdoc16", vOs.getPk_defdoch16());
  headVO.setAttributeValue("pk_defdoc17", vOs.getPk_defdoch17());
  headVO.setAttributeValue("pk_defdoc18", vOs.getPk_defdoch18());
  headVO.setAttributeValue("pk_defdoc19", vOs.getPk_defdoch19());
  headVO.setAttributeValue("pk_defdoc20", vOs.getPk_defdoch20());
...
/**
 * 功能描述:出库单暂估时向存货核算系统传送数据(成批单据传送)
 * 王凯飞
 */
private void saveBillFromOutter_saleyf(saleEstimateVO VOs[], String cOperator, UFDate dCurrDate,ClientLink cl) throws BusinessException {
    IIAToPUBill myService = null;
	IBill bo = null;
	...
	try {
		...
		BillVO billVOs[] = transferIAData_saleyf(VOs, cOperator, dCurrDate);
...
/**
 * 功能:依据暂估方式不同,向存货核算系统传递数据;单到回冲,传送暂估出库单;单到补差,传送采购入库单(成批单据传送)
 * 输入:暂估VO[],当前操作员,当前日期
 * 返回:BillVO[]
 * 异常:javax.naming.NamingException, java.rmi.RemoteException, java.sql.SQLException
 * 作者:王凯飞
 */
private BillVO[] transferIAData_saleyf(EstimateVO[] vOs, String cOperator, UFDate dCurrDate)
	throws BusinessException {
	nc.bs.pu.pub.PubImpl myPubService = null;
	if (vOs == null || vOs.length == 0) {
		SCMEnv.out("传入参数为空，直接返回NULL；调用堆栈如下：");
 ...
 * 作者:王凯飞
 */
private BillVO[] transferIAData_saleyf(saleEstimateVO[] vOs, String cOperator, UFDate dCurrDate)
	throws BusinessException {
	nc.bs.pu.pub.PubImpl myPubService = null;
	if (vOs == null || vOs.length == 0) {
		SCMEnv.out("传入参数为空，直接返回NULL；调用堆栈如下：");
    ...
    billVO = new BillVO[vOs.length];
    for(int i = 0; i < vOs.length; i++){
    	icVOs[i] = switchVOFromEstimate2IC_saleyf(vOs[i]);
    ...
    Vector vTemp = new Vector();
      ...
      bodyVO[0].setVsourcebillcode(vOs[i].getVbillcode());
			...
			if (d2 == null)
				d2 = new UFDouble(0.0);
			//double d = d1.doubleValue() + d2.doubleValue();
			bodyVO[0].setNmoney(d1);//setNmoney(new UFDouble(PubDMO.getRoundDouble(nMnyDecimal[0], d)));
			bodyVO[0].setNdrawsummny(d2);

			
			  //zhwj 暂估单价
			  UFDouble zgprice = vOs[i].getNprice();//getZGPrice(vOs[i].getPk_invmandoc());
			 UFDouble nnumber = new UFDouble(1); //vOs[i].getNoutnum()==null?new UFDouble(0):vOs[i].getNoutnum();
//			  bodyVO[0].setNmoney( new UFDouble(PubDMO.getRoundDouble(nMnyDecimal[0], nnumber.multiply(zgprice).doubleValue())));
			  bodyVO[0].setNmoney( new UFDouble(nnumber.multiply(zgprice).doubleValue()));
			  
			
			d1 = bodyVO[0].getNmoney();
			d2 = new UFDouble(1);//bodyVO[0].getNnumber();
			if (d2 != null && d2.doubleValue() != 0.0) {
				double d = d1.doubleValue() / d2.doubleValue();
				bodyVO[0].setNprice(new UFDouble(d));
			}
    ...
    vTemp = new Vector();
		for (int i = 0; i < billVO.length; i++) {
			BillHeaderVO headVO = (BillHeaderVO)billVO[i].getParentVO();
				...
				headVO.setCbilltypecode("IA");
			
			headVO.setCbillid(vOs[i].getCgeneralhid());

			headVO.setDbilldate(dCurrDate);
			headVO.setCsourcemodulename("SO");
			headVO.setFdispatchflag(new Integer(0));
 ...
 * @exception java.rmi.RemoteException 异常说明。
 */
public saleEstimateVO[] queryEstimate_saleyf(String unitCode, ConditionVO conditionVO[], String sZG, String sEstPriceSource)
	throws BusinessException {

	saleEstimateVO[] estimates = null;
	int nPriceDecimal = 2;
	int nMoneyDecimal = 0;
    ...
    String strDataPowerSql = (String) listRet.get(1);
		...
		for (int i = 0; i < conditionVO.length; i++) {
			...
			//出库单头自定义项
				...
				try {
					nc.bs.ps.estimate.saleEstimateDMO ddmo = new nc.bs.ps.estimate.saleEstimateDMO();
					String sClassCode[] = ddmo.getSubInvClassCode(sValue);
    ...
    if(strDataPowerSql != null) sCondition += " and " + strDataPowerSql;
		...
		saleEstimateDMO dmo = new saleEstimateDMO();
		estimates = dmo.queryEstimate_saleyf(unitCode, sCondition, sZG);
		...
		*/
		...
		if(estimates != null && estimates.length > 0) {
			...
			Vector vTemp = new Vector();
			for(int i = 0; i < estimates.length; i++){
				if(estimates[i].getCfirsttype() != null && estimates[i].getCfirsttype().equals(ScmConst.SO_Order)){
					if(!vTemp.contains(estimates[i].getCfirstbillbid())) vTemp.addElement(estimates[i].getCfirstbillbid());
				}
			}
			if(vTemp.size() > 0){
				String sID[] = new String[vTemp.size()];
				vTemp.copyInto(sID);
				Hashtable hTemp = dmo.queryOrderTaxRate_sale(sID);
				if(hTemp != null && hTemp.size() > 0){
					for(int i = 0; i < estimates.length; i++){
						if(estimates[i].getCfirsttype() != null && estimates[i].getCfirsttype().equals(ScmConst.SO_Order) && hTemp.get(estimates[i].getCfirstbillbid()) != null){
							Object oTemp[] = (Object[])hTemp.get(estimates[i].getCfirstbillbid());
							UFDouble nTaxRate = new UFDouble(0);
							Integer iDiscountTaxType = new Integer(1);
							if(oTemp[0] != null) nTaxRate = new UFDouble(oTemp[0].toString());
 ...
 * 作者：王凯飞
 */
public void antiEstimate_saleyf(saleEstimateVO VOs[], String cOperator)
	throws BusinessException {
	saleEstimateDMO dmo = null;
	ICreateCorpQueryService myService0 = null;
	boolean bLock = false;
	...
	//组合所有需要加锁的主键
	String sKeys[] = getEstimateLockKeys_sale(VOs);

	try {
		dmo = new saleEstimateDMO();
		//对单据加锁
		bLock = LockTool.setLockForPks(sKeys, cOperator);
		if (!bLock) {
		...
		//判断时间戳是否改变
		String sMessage = isTimeStampChanged_so(VOs);
		if (sMessage != null && sMessage.length() > 0) {
			sMessage += "请刷新！\n";
			throw new BusinessException(sMessage);
		}
...
//		}
		//
		timerDebug.addExecutePhase("出库单表体打上正常标志，重新计算暂估单价和暂估金额及材料费");
		...
		if (VOs != null && VOs.length > 0) {
			...
			if (bIAStartUp) {
				...
				timerDebug.addExecutePhase("按出库单“表头ID[]”删除存货核算单据");
				...
				timerDebug.addExecutePhase("按出库单“表体ID[]”删除存货核算单据");
			}
			...
			*/
			deleteBillForZGAP_saleyf(VOs);
			
			//zhwj 回写
			dmo.backICStatus_so(VOs, status[0], "");
...
//王凯飞
//反暂估
private void deleteBillForZGAP_saleyf(saleEstimateVO[] vOs)
throws BusinessException {
	Vector v1 = new Vector();
	for(int i = 0; i < vOs.length; i++){
//		if(VOs[i].getBzgyf().booleanValue()) //zhwj
...
//			bodyVO.setDdhh("1016A2100000000FMPTG");
//			bodyVO.setDdlx("1016A2100000000FMP9J");
...
/*
 * 暂估VO转换为库存出库单VO,服务于暂估VO对照
 * 王凯飞
 */
private GeneralBillVO switchVOFromEstimate2IC_saleyf(saleEstimateVO estVO){
  GeneralBillItemVO bodyVO = new GeneralBillItemVO();
  
  bodyVO.setCgeneralbid(estVO.getCgeneralbid());
  bodyVO.setCgeneralhid(estVO.getCgeneralhid());

  bodyVO.setCinventoryid(estVO.getPk_invmandoc());
  bodyVO.setVbatchcode(estVO.getVbatchcode());
//  bodyVO.setNinnum(estVO.getNoutnum());  
  bodyVO.setNoutnum(estVO.getNoutnum());
  bodyVO.setPk_cubasdoc(estVO.getPk_cubasdoc());
	//获取本位币精度
	nc.bs.pu.pub.PubImpl myPubService = new nc.bs.pu.pub.PubImpl();
	int nMnyDecimal[];
	try {
		...
		if (nMnyDecimal == null || nMnyDecimal.length == 0) {
		  ...
		  //zhwj 暂估单价
		  UFDouble zgprice = estVO.getNprice()==null?new UFDouble(0):estVO.getNprice();//getZGPrice(estVO.getPk_invmandoc());
		  
		 UFDouble nnumber = new UFDouble(1);//estVO.getNoutnum()==null?new UFDouble(0):estVO.getNoutnum();
		 UFDouble ntotalmoney = zgprice.multiply(nnumber);
...
//  bodyVO.setNprice(estVO.getNprice());
  ...
  bodyVO.setCaccountunitid(estVO.getCastunitid());
  bodyVO.setHsl(estVO.getHsl());
//  bodyVO.setNinassistnum(estVO.getNoutassistnum());
  bodyVO.setNoutassistnum(estVO.getNoutassistnum());
  bodyVO.setCfirstbillhid(estVO.getCfirstbillhid());
  bodyVO.setCfirstbillbid(estVO.getCfirstbillbid());
  bodyVO.setCfirsttype(estVO.getCfirsttype());
  ...
  headVO.setCbilltypecode(estVO.getCbilltypecode());
  headVO.setCgeneralhid(estVO.getCgeneralhid());
  headVO.setCcustomerid(estVO.getCcustomerid());
  headVO.setCdispatcherid(estVO.getCdispatcherid());
  headVO.setCbiztypeid(estVO.getCbiztype());
  headVO.setPk_calbody(estVO.getCcalbodyid());
  headVO.setCwarehouseid(estVO.getCwarehouseid());
...
public void saveBillForARAP_yf(EstimateVO[] vOs, ArrayList paraList) throws BusinessException {
    ...
    DJZBVO arapVO[] = (DJZBVO[]) PfUtilTools.runChangeDataAry("4C", "D1", icVOs);
    if(arapVO == null || arapVO.length == 0) return;
 ...
 * 王凯飞
 */
public void saveBillForARAP_saleyf(saleEstimateVO[] vOs, ArrayList paraList) throws BusinessException {
	String sCurrTypeID = (String)paraList.get(0);
	String cOperator = (String)paraList.get(1);
	UFDate dCurrDate = (UFDate)paraList.get(2);
	nc.vo.scm.pu.Timer timer = new nc.vo.scm.pu.Timer();
	...
	try {
		...
		//暂估应付时,客商取订单发票方,发票方不存在,取客商 2006-08-25
		/*
		Vector vTemp = new Vector();
		for(int i = 0; i < vOs.length; i++){
			String s = vOs[i].getCfirstbillhid();
		...
		*/
		...
		String pk_cumandoc[] = new String[vOs.length];
		for(int i = 0; i < vOs.length; i++) pk_cumandoc[i] = vOs[i].getCcustomerid();
		SettleDMO settleDMO = new SettleDMO();
		String pk_cubasdoc[] = settleDMO.queryVendorBaseIDForARAP(pk_cumandoc);
		if (pk_cubasdoc != null && pk_cubasdoc.length > 0) {
			for (int i = 0; i < vOs.length; i++) {
				vOs[i].setPk_cubasdoc(pk_cubasdoc[i]);
			}
		}
		//查询存货基本ID
		Vector v1 = new Vector();
		for(int i = 0; i < vOs.length; i++){
			if(vOs[i].getCbaseid() == null) v1.addElement(vOs[i].getPk_invmandoc());
		}
		if(v1.size() > 0){
			pk_cumandoc = new String[v1.size()];
			v1.copyInto(pk_cumandoc);
    ...
    DJZBVO[] arapVOs = new DJZBVO[icVOs.length];
...
//    GeneralBillHeaderVO hfromVO = new GeneralBillHeaderVO();
    for (int i = 0; i < icVOs.length; i++) {
		...
		hvo.setSfkr(hfromVO.getCcustomerid());
...
//    DJZBVO arapVO[] = (DJZBVO[]) PfUtilTools.runChangeDataAry("4C", "D1", icVOs);
    if(arapVOs == null || arapVOs.length == 0) return;
		...
		for (int i = 0; i < vOs.length; i++) {
			//后续处理:应付单头
			DJZBHeaderVO headVO = (DJZBHeaderVO)arapVOs[i].getParentVO();
			headVO.setVouchid(vOs[i].getCgeneralhid());
			...
			headVO.setPzglh(new Integer(1));
			headVO.setWldx(new Integer(1));
//			headVO.setSfkr("0001A210000000003K0H");
			//zhwj 运费暂估 应付单增加标识，反暂时需要处理
			headVO.setZyx29("Y");
			headVO.setScomment("价外费暂估自动生成"); 
			//
			...
			//后续处理:应付单体
			DJZBItemVO bodyVO[] = (DJZBItemVO[])arapVOs[i].getChildrenVO();
			bodyVO[0].setDwbm(vOs[i].getPk_corp());
			bodyVO[0].setFb_oid(vOs[i].getCgeneralbid());

			bodyVO[0].setHbbm(vOs[i].getPk_cubasdoc()); //客商基本ID
			bodyVO[0].setKsbm_cl(vOs[i].getCcustomerid());//
			bodyVO[0].setCinventoryid(vOs[i].getCbaseid()); //存货基本ID
			...
			*/
			...
			if (nMnyDecimal == null || nMnyDecimal.length == 0) {
			  ...
			  //zhwj 暂估单价ntotalmoney
			 UFDouble zgprice = vOs[i].getNprice()==null?new UFDouble(0):vOs[i].getNprice();//getZGPrice(vOs[i].getPk_invmandoc());
			 UFDouble nnumber = new UFDouble(1);//vOs[i].getNoutnum()==null?new UFDouble(0):vOs[i].getNoutnum();
			 UFDouble ntotalmoney = zgprice.multiply(nnumber);
			 ...
			 ntotalmoney = new UFDouble(PubDMO.getRoundDouble(nMnyDecimal[0], ntotalmoney.doubleValue()));
				...
				//含税单价
				bodyVO[0].setHsdj(zgprice);
//				bodyVO[0].setHbbm("0001A210000000003UAF");
//				bodyVO[0].setKsbm_cl("0001A2100000000040F2");
//			
//			//价税合计
//			bodyVO[0].setDfbbje(VOs[i].getNtotalmoney());
//			bodyVO[0].setDfybje(VOs[i].getNtotalmoney());
...
//			bodyVO[0].setHsdj(VOs[i].getNtaxprice());
			//zhwj 挂起标志  设置后系统应付单会挂起，不能弃审？
			bodyVO[0].setPausetransact(new UFBoolean(true));
			bodyVO[0].setOthersysflag("销售出库暂估");
	...
	private DJZBItemVO[] switchVOIcToDJZBb(GeneralBillHeaderVO hvo,GeneralBillItemVO[] bvo) {
		...
		/*
		 * "H_xslxbm->H_cbiztype", "H_dwbm->H_pk_corp", "B_ph->B_cfirsttype", "B_deptid->H_cdptid", "B_dfybwsje->B_nmny",
		 *  "B_xmbm2->B_cprojectid", "B_jsfsbm->H_cbilltypecode", "B_vouchid->B_cgeneralhid", "B_cksqsh->B_cfirstbillbid",
		 *   "B_ybye->B_nmny", 
            "B_xyzh->B_cfirstbillhid", "B_dfbbwsje->B_nmny", "B_ddhh->B_cgeneralbid", "B_shlye->B_ninnum", "B_dfybje->B_nmny", 
            "B_pk_jobobjpha->B_cprojectphaseid", "B_chbm_cl->B_cinventoryid", "B_ddlx->B_cgeneralhid", "B_bbye->B_nmny", 
            "B_dfshl->B_ninnum", 
            "B_ksbm_cl->H_cproviderid", "B_dfbbje->B_nmny", "B_dj->B_nprice", "B_seqnum->B_vbatchcode"*/
		for (int i = 0; i < bvo.length; i++) {
			...
			itemvo.setShlye(bvo[i].getNoutnum());
			...
			itemvo.setDfshl(bvo[i].getNoutnum());
			...
			itemvo.setHbbm(bvo[i].getPk_cubasdoc());
			itemvo.setKsbm_cl(bvo[i].getPk_cubasdoc());
			bItemvo[i]=itemvo;
		}
		return bItemvo;
	}
...
public void estimate_saleyf(saleEstimateVO[] VOs, ArrayList paraList,
	...
	//组合所有需要加锁的主键
	String sKeys[] = getsaleEstimateLockKeys(VOs);
	...
	try {
		...
		//状态刷新：判断是否存在已经暂估的库单行(存在抛异常)
		if (v1.size() > 0) {
			dmo.checkExistEsti_yf(v1);
		}
		timer.addExecutePhase("状态刷新：判断是否存在已经暂估的出库单行(存在抛异常)");

		//暂估时单价和金额的精度控制
		myService2 = new nc.bs.pu.pub.PubImpl();
		int digit[] = myService2.getDigitBatch(VOs[0].getPk_corp(), new String[] { "BD301", "BD505" });
		if (digit == null || digit.length == 0)
			throw new BusinessException("获取本位币精度或单价精度异常!");
...
*/
		...
		if (VOs != null && VOs.length > 0) {
			...
			myService1 = (ISysInitQry)nc.bs.framework.common.NCLocator.getInstance().lookup(ISysInitQry.class.getName());
			boolean bStockRB = true; //出库单是否红蓝对冲
			nc.vo.pub.para.SysInitVO initVO[] = myService1.querySysInit(unitCode, "PO36");
			if (initVO != null && initVO.length > 0) {
				if (initVO[0].getValue().equals("否"))
					bStockRB = false;
			}
			timer.addExecutePhase("出库单是否红蓝对冲");
			/*
			if (bIAStartUp) {
				//调用接口,向存货核算系统传送数据   生成存货核算的 出库调整单
				saveBillFromOutter_saleyf(VOs, cOperator, dCurrDate,cl);
				timer.addExecutePhase("调用接口,向存货核算系统传送数据[总时间]");
//				//结算
//				if (bStockRB) {
//					saSubLockedId = settleForEstimate(VOs, cOperator, dCurrDate, sEstMode, sDifferMode);
...
//				timer.addExecutePhase("结算");
			...
			*/
			...
			if(bZGYF.booleanValue() && myService0.isEnabled(unitCode, "AP")){
				...
				list.add(cOperator);
				list.add(dCurrDate);
				saveBillForARAP_saleyf(VOs,list);
			}
			
			//回写
			dmo.backICStatus_sale(VOs, status[1], dCurrDate.toString());
