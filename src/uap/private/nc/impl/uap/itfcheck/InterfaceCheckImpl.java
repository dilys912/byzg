...
//			svo = recordInsertVO(xre.getServiceId(), xre.getMsgBody(),new Integer(-1), message + data,corp,erpBillCode,bcBillCode);
	...
	/*//edit by zwx 2019-11-17 发送记录表增加公司、ERP单据号、标财单据号
	*//**
	 * @param billcode 单据号
	 * @param val 消息体
	 * @param in 0==成功   1==失败
...
//					json.put(fcode, v.trim());
					//edit by zwx 2019-10-12 存在包含与不包含小数点问题
					if(v.trim().contains(".")){
						json.put(fcode, v.trim());
					}else{
		...
		if("C".equals(ftype)){
			...
			if(ns.length<2){//整数
				...
				if(zs!=null&&zs.length()>0){
					if(Integer.valueOf(zs)>0||Integer.valueOf(zs)==0){//add by zsh 2019年10月11日 判断是否是正负数
...
//				val = zss+xss;
				...
				String zs = ns[0];
				String xs = ns[1];
				if(Integer.valueOf(zs)>0||Integer.valueOf(zs)==0){//判断是否是正负数
