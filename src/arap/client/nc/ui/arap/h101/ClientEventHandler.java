...
//	
	...
	protected void onBoSave() throws Exception {
		...
		rollback.append(" update arap_djfb set zyx9 = '1' where (dwbm = '1078' or dwbm = '1108') and fb_oid in ("+djfbzj.toString()+")  and nvl(dr,0)=0 ;");
		...
		for (int i = 0; i < pk_bt.size(); i++) {
			...
			.append("  (dwbm = '1078' or dwbm = '1108') and and  nvl(zyx9,0) <> '1' and nvl(dr,0)=0 ; "); 
			...
			if(a == null ||a.size() <= 0){
				...
				rollbackzb2.append(" update arap_djzb set zyx9 = '1' where vouchid =( ") 
					.append(" select vouchid from arap_djfb where  (dwbm = '1078' or dwbm = '1108') and fb_oid = '"+zj2+"')")
					.append("  (dwbm = '1078' or dwbm = '1108') and and  nvl(zyx9,0) <> '1' and nvl(dr,0)=0 ; "); 
	...
	protected void onBoDelete() throws Exception {
		...
		//0表示未生成
		StringBuffer rollback = new StringBuffer();
		rollback.append(" update arap_djfb set zyx9 = '0' where (dwbm = '1078' or dwbm = '1108') and fb_oid in ("+djfbzj.toString()+")  and nvl(dr,0)=0 ;");
		
		ipubdmo.executeUpdate(rollback.toString());
		//回写表头未生成
		StringBuffer rollbackzb1 = new StringBuffer();
		rollbackzb1.append(" select distinct vouchid from arap_djfb where (dwbm = '1078' or dwbm = '1108') and fb_oid in ("+djfbzj.toString()+") and nvl(dr,0)=0 ; "); 
		List a = (List) sessionManager.executeQuery(rollbackzb1.toString(),new MapListProcessor());
		...
		if(a.size()>0){
			...
			StringBuffer rollbackzb2 = new StringBuffer();
			rollbackzb2.append(" update arap_djzb set zyx9 = '0' where  (dwbm = '1078' or dwbm = '1108') and vouchid in ("+djzbzj.toString()+") ")
