 ...
 */
public EstimateDMO() throws javax.naming.NamingException, SystemException {
	super();
}
/**
 * EstimateDMO 构造子注解。
 ...
 * @exception nc.bs.pub.SystemException 父类构造子抛出的异常。
 */
public EstimateDMO(String dbName) throws javax.naming.NamingException, SystemException {
	super(dbName);
}
	...
	try {
      ...
      estimate.setBcalculatecost(strTmp == null ? new UFBoolean(false) : new UFBoolean(strTmp.trim()));      
		...
		if (vv.size() > 0) {
			...
			Vector vTemp = new Vector();
			vTemp.addElement(((EstimateVO) vv.elementAt(0)).getCmangid());
			for (int i = 1; i < vv.size(); i++) {
				if (!vTemp.contains(((EstimateVO) vv.elementAt(i)).getCmangid()))
					vTemp.addElement(((EstimateVO) vv.elementAt(i)).getCmangid());
			}
			String sTemp[] = new String[vTemp.size()];
			vTemp.copyInto(sTemp);
			//临时表
			...
			*/
			...
			for (int i = 0; i < vv.size(); i++) {
				EstimateVO tempVO = (EstimateVO) vv.elementAt(i);
				if (vMangID.contains(tempVO.getCmangid())) {
					int j = vMangID.indexOf(tempVO.getCmangid());
					if (j >= 0)
