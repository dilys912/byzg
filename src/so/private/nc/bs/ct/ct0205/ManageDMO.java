...
public class ManageDMO extends ContractDMO
{
  ...
  {
    beforeCallMethod("nc.bs.ct.ct0205.ManageDMO", "queryAllBodyData", new Object[] { key });

    StringBuffer baseSQL = new StringBuffer(1260);
    ManageItemVO[] voaBody = null;
    if(!getParentCorp("1016").equals("10395"))
    {
    	...
    	.append(" ct_b.def2, ct_b.def3, ct_b.def4, ct_b.def5, ct_b.def6, ct_b.def7, ct_b.def8, ct_b.def9, ct_b.def10,ct_b.ts,ct_b.crowno,ct_b.bsafeprice,ct_b.breturnprofit,ct_b.cpricetableid, ")
    	...
    	.append(" ct_b.vfree1, ct_b.vfree2, ct_b.vfree3, ct_b.vfree4, ct_b.vfree5, ")
    	.append(" ct_b.creceiptcorpid, ct_b.vreceiveaddress, ct_b.creceiptareaid, ct_b.crecaddrnodeid ")
    	...
    	.append(key)
    	.append("'")
    	.append(" and (inv.iscansold = 'Y' or ct_b.invid is null) ");
    }
    else
    {
    	baseSQL.append(" SELECT ct_b.pk_ct_manage_b, ct_b.pk_ct_manage, ct_b.pk_corp, ct_b.invid,ct_b.invclid, ct_b.delivdate, ct_b.amount,")
    	...
    	.append(" ct_b.pk_defdoc11, ct_b.pk_defdoc12, ct_b.pk_defdoc13, ct_b.pk_defdoc14, ct_b.pk_defdoc15, ct_b.pk_defdoc16, ct_b.pk_defdoc17, ct_b.pk_defdoc18, ct_b.pk_defdoc19, ct_b.pk_defdoc20, ")
    	.append(" ct_b.vfree1, ct_b.vfree2, ct_b.vfree3, ct_b.vfree4, ct_b.vfree5, ")
    	.append(" ct_b.creceiptcorpid, ct_b.vreceiveaddress, ct_b.creceiptareaid, ct_b.crecaddrnodeid,ct_b.tsbl,ct_b.dpzk ")
    	.append(" FROM ct_manage_b ct_b INNER JOIN ct_manage ct ON ct_b.pk_ct_manage = ct.pk_ct_manage ")
    	.append(" INNER JOIN ct_type type ON ct.pk_ct_type = type.pk_ct_type ")
    	.append(" left outer join bd_invmandoc inv on ct_b.invid = inv.pk_invmandoc ")
    	.append(" WHERE ct_b.dr = 0  and ")
    ...
    afterCallMethod("nc.bs.ct.ct0205.ManageDMO", "queryAllBodyData", new Object[] { key });
    ...
    throws BusinessException
  {
    ...
    whereString = " type.nbusitype=1 and ct.ctflag = 2 and ct.ifearly='N' and ( " + whereString + " )";
    return super.queryAllHeadData(whereString);
  }
  public String getParentCorp(String key)
  {
	  String ParentCorp = new String();
	
		try {
			CorpVO corpVO = CorpBO_Client.findByPrimaryKey(key);
			ParentCorp = corpVO.getFathercorp();
			CorpVO pcorpVO = CorpBO_Client.findByPrimaryKey(ParentCorp);
			ParentCorp =pcorpVO.getUnitcode();
