...
public class ShowResult extends nc.ui.pub.beans.UIDialog implements java.awt.event.ActionListener, MouseListener{
	...
	private InvOnHandItemVO[] results;
	
	public InvOnHandItemVO[] getResults() {
		return results;
	}
	public void setResults(InvOnHandItemVO[] results) {
		this.results = results;
	}
	public ShowResult(ToftPanel toftPanel,InvOnHandVO vo ) {
		...
		if (tablemodel == null) {
			try {
				...
				if(vo!=null && vo.getChildrenVO()!=null &&  vo.getChildrenVO().length>0){
					data=new String[vo.getChildrenVO().length][cname.length];
					InvOnHandItemVO item = null;
					String temp = null;
					SelectVOtb=new Hashtable();
					//sortFile=vo.getChildrenVO()
					for (int i = 0; i < vo.getChildrenVO().length; i++) {
						item = (InvOnHandItemVO) vo.getChildrenVO()[i];
						//if(sortFile==null||sortFile.length<=0)
					//	{
						//	sortFile=new String[item.getAttributeNames().length];
						//	sortFile=item.getAttributeNames();
					//	}
						SelectVOtb.put(String.valueOf(i+1), item);
						data[i][0]=String.valueOf(i+1);
						
						data[i][1]=item.getAttributeValue("bodyname")==null?"":item.getAttributeValue("bodyname").toString();
						data[i][2]=item.getAttributeValue("storname")==null?"":item.getAttributeValue("storname").toString();
						data[i][3]=item.getAttributeValue("invname")==null?"":item.getAttributeValue("invname").toString();
//						date[i][3]=vo.getChildrenVO()[i].getAttributeValue("cwarehouseid")==null?"":vo.getChildrenVO()[i].getAttributeValue("cwarehouseid").toString();
						if(item.getNum()!=null)
						{
							data[i][4]=item.getNum().toString();
						}
						data[i][5] = item.getVbatchcode();
						if(item.getFreeItemVO()!=null)
						{
							data[i][6] = item.getFreeItemVO().getVfree1();
						}
//						date[i][4]=vo.getChildrenVO()[i].getAttributeValue("pk_invbasdoc")==null?"":vo.getChildrenVO()[i].getAttributeValue("pk_invbasdoc").toString();
					}
				}else{
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		/*		vo.setAttributeValue("bodyname", jtable.getValueAt(ii[i],0));
			...
			///	Object //freevo;
				...
				vo=(InvOnHandItemVO) SelectVOtb.get(key);
				vo.setAssistnum(new UFDouble(key));
				vos[i]=vo;
			}
			this.setResults(vos);
			this.closeOK();
