...
public class ShowSalesOutResult extends nc.ui.pub.beans.UIDialog implements java.awt.event.ActionListener, MouseListener{
...
//	InvOnHandItemVO[]result=null;
	...
	public ShowSalesOutResult(ToftPanel toftPanel,ArrayList list ) {
...
//				String cname[]=new String[]{"客户","仓库编号","产品名称","存货主键","批次号","表体主键","垛号","数量"};
				String cname[]=new String[]{"客户","仓库编号","产品名称","存货主键","批次号","","垛号","数量"};
				String cname_en[]=new String[]{"Customer", "Warehouse number", "Product Name", "Inventory master key", "Batch number", "StackNo.", "Quantity"};
				String [][] date;
				int length=0;
				...
				if(list!=null && list.size()>0){
					...
					for (int i = 0; i < list.size(); i++) {
						...
						for (int j = 0; j < vo.getChildrenVO().length; j++) {
							...
							date[j+index][5]=vo.getChildrenVO()[j].getPrimaryKey();
							date[j+index][6]=item.getVfree1(); //垛号
							if(item.getNoutnum()!=null)
							{
								date[j+index][7]=item.getNoutnum().toString();//数量
							}
						}
				...
				tablemodel.setDataVector(date, ClientEnvironment.getInstance().getLanguage().equalsIgnoreCase("simpchn")?cname:cname_en);
//				jtable.getTableHeader().getColumnModel().getColumn(5).setMaxWidth(0); 
//				jtable.getTableHeader().getColumnModel().getColumn(5).setWidth(0); 
	...
	private nc.ui.pub.beans.UITable getUILabel1() {
		if (jtable == null) {
			try {
				...
				jtable.getTableHeader().getColumnModel().getColumn(5).setMaxWidth(0); 
				jtable.getTableHeader().getColumnModel().getColumn(5).setWidth(0);
				jtable.getTableHeader().getColumnModel().getColumn(5).setPreferredWidth(0);
...
//				vos[i]=jtable.getModel().getValueAt(ii[i],4).toString();
				vos[i]=jtable.getModel().getValueAt(ii[i],5).toString();//使用表体主键
			}
			this.result=vos;
