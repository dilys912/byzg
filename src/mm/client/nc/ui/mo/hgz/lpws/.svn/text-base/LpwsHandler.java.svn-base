...
public class LpwsHandler extends ManageEventHandler{
	
	//add by zy 2019-08-26
	private ClientEnvironment ce = ClientEnvironment.getInstance();
	//end by zy

	public LpwsHandler(BillManageUI billUI, IControllerBase hgzController) {
	...
	protected void onBoSave() throws Exception{
		...
		//add by zy 2019-09-29  增加品管员和数量非空验证
...
//        	  nc.ui.pub.print.IDataSource dataSource = new ListPanelPRTS(getBillUI()._getModuleCode(), ((BillManageUI)getBillUI()).getBillListPanel());
	...
	protected void CreateLinCheck(){
		...
		HgzHeadVO hgzVO=null;
		 int[] rows = ((LpwsUI) getBillUI()).getBillListPanel().getHeadTable().getSelectedRows();
...
//			getBillCardPanelWrapper().getBillCardPanel().getHeadItem("nnumber").setValue(hgz.getNnumber());//数量  editor by zy 2019-09-29 新增单据时不带出上个单据历史数量
