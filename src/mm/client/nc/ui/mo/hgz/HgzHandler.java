...
public class HgzHandler extends ManageEventHandler{
	
	//add by zy 2019-08-26
	private ClientEnvironment ce = ClientEnvironment.getInstance();
	//end by zy

	public HgzHandler(BillManageUI billUI, IControllerBase hgzController) {
	...
	protected void onBoSave() throws Exception{
		...
		//end
		String coacode = getBillCardPanelWrapper().getBillCardPanel().getHeadItem("coacode").getValueObject()==null?
				"": getBillCardPanelWrapper().getBillCardPanel().getHeadItem("coacode").getValueObject().toString();
		if(coacode.length()==0){
			getBillUI().showWarningMessage("COA编码必填！");
			return;
		}
		String tph = getBillCardPanelWrapper().getBillCardPanel().getHeadItem("palletnumber").getValueObject()==null?
				"": getBillCardPanelWrapper().getBillCardPanel().getHeadItem("palletnumber").getValueObject().toString();
		if(tph.length()==0){
			getBillUI().showWarningMessage("托盘号必填！");//必填托盘号否则影响后续截取字符串
			return;
		}else if(tph.length()!=9){
			getBillUI().showWarningMessage("托盘号只允许9位数！");//必填托盘号否则影响后续截取字符串
			return;
		}
		String vdef8 = getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdef8").getValueObject()==null?
				"": getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdef8").getValueObject().toString();
		if(vdef8.length()>0){
			if(!isNumeric(vdef8)){
				getBillUI().showWarningMessage("流水号必须为正整数！");
				return;
			}
			if(Integer.valueOf(vdef8)>999){
				getBillUI().showWarningMessage("流水号必须小于999");
				return;
			}
...
//		getBillCardPanelWrapper().getBillCardPanel().getHeadItem("palletnumber").setValue(tph);
	 ...
	 * @throws Exception 
	 * 
	 * sap规则：1-8：sap物料编码 固定
	 * 9-15：工厂编码
	 * 16-20：生产日期（年末尾数+月+日）
	 * 21-22：班次
	 * 23-24：机台
	 * 25-29：（年末尾数+月+日）生产日期+一年半
	 * 30-32：流水号：根据机台+年份，001-999，如果当年超过999则按照001重新开始
	 */
	protected void createSapCode() throws Exception{
		...
		String zsdh = getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdef8").getValueObject()==null?"":getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdef8").getValueObject().toString();
		if(zsdh.length()==0){
			throw new BusiBeanException("流水号不能为空，请输入后再试！");
		}
...
//        	  nc.ui.pub.print.IDataSource dataSource = new ListPanelPRTS(getBillUI()._getModuleCode(), ((BillManageUI)getBillUI()).getBillListPanel());
	...
	protected void CreateLinCheck(){
		...
		HgzHeadVO hgzVO=null;
		 int[] rows = ((HgzUI) getBillUI()).getBillListPanel().getHeadTable().getSelectedRows();
...
//			getBillCardPanelWrapper().getBillCardPanel().getHeadItem("remarks").setValue(hgz.getRemarks());//备注
			...
			getBillCardPanelWrapper().getBillCardPanel().getHeadItem("vdef8").setValue(getNewSerialNum(hgz.getVdef8()==null?"":hgz.getVdef8()));//流水号
		}
	}
	...
	//add by gt 2019-08-12   下线检验单，是否并垛点击‘是’后，调用现存量查询方法，弹出现存量查询的窗口
	   public  void onQueryProducts(String zdslnum, String cpid, String pgy,ArrayList handvoList,String mplatform) throws Exception {
			...
			dlg.onQuery2(zdslnum, cpid, pgy,handvoList,scx);
			/*if (result != null) {
