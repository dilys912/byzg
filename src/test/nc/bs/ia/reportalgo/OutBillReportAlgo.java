...
public class OutBillReportAlgo extends BaseReportAlgo
{
    ...
    throws BusinessException
  {
    ...
    if ((((sDime.indexOf("dbilldate") >= 0) || (sDime.indexOf("vbillcode") >= 0) || (sDime.indexOf("vbomcode") >= 0) || (sWhere.indexOf("dbilldate") >= 0) || (sWhere.indexOf("dauditdate") >= 0) || (sWhere.indexOf("vbillcode") >= 0) || (sWhere.indexOf("vbomcode") >= 0) || (sWhere.indexOf("bdef") >= 0) || (sWhere.indexOf("vdef") >= 0))) && (queryVO.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)))
    {
    ...
    if ((((sCorpAndPeriod[1] == null) || (sCorpAndPeriod[1].length() == 0))) && (((sCorpAndPeriod[2] == null) || (sCorpAndPeriod[2].length() == 0))) && (queryVO.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)))
    {
  ...
  private String getSQLForBill(QueryVO query, String sCorpAndPeriod)
  {
    ...
    if (query.isShowSetPart()) {
      ...
      strSql.append(" else setpart.childsnum * coalesce(b.nnumber, 0) end) as noutnum ,");
      strSql.append(" sum(case when setpart.pk_invmandocset is null then coalesce(b.nmoney, 0) ");
      strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(b.nmoney, 0) end) as noutmny , ");
      strSql.append(" sum(case when setpart.pk_invmandocset is null then coalesce(b.nplanedmny, 0) ");
      strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(b.nplanedmny,0) end) as noutplanedmny ,");
      strSql.append(" sum(case when setpart.pk_invmandocset is null then coalesce(b.noutvarymny, 0) ");
      strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(b.noutvarymny,0) end) as noutvarymny ");

      if (query.isShowAssistant()) {
        strSql.append(", sum(case when setpart.pk_invmandocset is null then coalesce(b.nassistnum, 0) ");
        strSql.append(" else setpart.childsnum * coalesce(b.nassistnum,0) end) as noutassistnum ");
      }

      if (query.isShowAdjustMoney()) {
        strSql.append(", sum(case when h.cbilltypecode='IA' then (case when setpart.pk_invmandocset is null then coalesce(b.nmoney, 0) ");
        strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(b.nmoney, 0) end) else 0 end) as noutadjustmny ");
      }

      if (query.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)) {
        strSql.append(", sum(case when h.bwithdrawalflag='Y' then (case when setpart.pk_invmandocset is null then coalesce(b.nnumber, 0) ");
        strSql.append(" else setpart.childsnum * coalesce(b.nnumber, 0) end) else 0 end) as njtlnum ");
      }
      ...
      strSql.append(", sum(coalesce(b.nnumber, 0)) as noutnum, sum(coalesce(b.nmoney, 0)) as noutmny, ");
      strSql.append(" sum(coalesce(b.nplanedmny, 0)) as noutplanedmny, sum(coalesce(b.noutvarymny, 0)) as noutvarymny ");

      if (query.isShowAssistant()) {
        strSql.append(", sum(coalesce(b.nassistnum, 0)) as noutassistnum ");
      }

      if (query.isShowAdjustMoney()) {
        strSql.append(", sum(case when h.cbilltypecode='IA' then coalesce(b.nmoney, 0) else 0 end) as noutadjustmny ");
      }

      if (query.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)) {
        strSql.append(", sum(case when h.bwithdrawalflag='Y' then coalesce(b.nnumber, 0) else 0 end ) as njtlnum ");
      }
    ...
    strSql.append(" and h.cbilltypecode in ('I5','I6','I7','I8','IA','IC','IJ') ");
  ...
  private String getSQLForPeriod(QueryVO query, String sCorpAndPeriod)
  {
    ...
    if (query.isShowSetPart()) {
      ...
      strSql.append(", sum(case when setpart.pk_invmandocset is null then coalesce(m.noutnum, 0) ");
      strSql.append(" else setpart.childsnum * coalesce(m.noutnum, 0) end) as noutnum, ");
      strSql.append(" sum(case when setpart.pk_invmandocset is null then coalesce(m.noutmny, 0) ");
      strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(m.noutmny, 0) end) as noutmny, ");
      strSql.append(" sum(case when setpart.pk_invmandocset is null then coalesce(m.noutplanedmny, 0) ");
      strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(m.noutplanedmny,0) end) as noutplanedmny, ");
      strSql.append(" sum(case when setpart.pk_invmandocset is null then coalesce(m.noutvarymny, 0) ");
      strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(m.noutvarymny,0) end) as noutvarymny ");

      if (query.isShowAssistant()) {
        strSql.append(", sum(case when setpart.pk_invmandocset is null then coalesce(m.noutassistnum, 0) ");
        strSql.append(" else setpart.childsnum * coalesce(m.noutassistnum,0) end) as noutassistnum  ");
      }

      if (query.isShowAdjustMoney()) {
        strSql.append(", sum(case when m.cbilltypecode='I9' then (case when setpart.pk_invmandocset is null then coalesce(m.noutmny, 0) ");
        strSql.append(" else setpart.childsnum * setpart.partpercent*coalesce(m.noutmny, 0) end) else 0 end) as noutadjustmny ");
      }

      if (query.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)) {
        strSql.append(", sum(case when m.bwithdrawalflag='Y' then(case when setpart.pk_invmandocset is null then coalesce(m.noutnum, 0) ");
        strSql.append(" else setpart.childsnum * coalesce(m.noutnum, 0) end) else 0 end) as njtlnum ");
      }
      ...
      sHaving = sHaving + " sum(case when setpart.pk_invmandocset is null then coalesce(m.noutnum, 0) ";
      sHaving = sHaving + " else setpart.childsnum * coalesce(m.noutnum, 0) end)!=0 ";
      sHaving = sHaving + " or sum(case when setpart.pk_invmandocset is null then coalesce(m.noutmny, 0) ";
      sHaving = sHaving + " else setpart.childsnum * setpart.partpercent*coalesce(m.noutmny, 0) end)!=0 ";
      sHaving = sHaving + " or sum(case when setpart.pk_invmandocset is null then coalesce(m.noutplanedmny, 0) ";
      sHaving = sHaving + " else setpart.childsnum * setpart.partpercent*coalesce(m.noutplanedmny,0) end)!=0 ";
    }
    else {
      strSql.append(SQLStringUtil.getDimensionSQL(query, 1, false, false));

      strSql.append(", sum(coalesce(m.noutnum, 0)) as noutnum, sum(coalesce(m.noutmny, 0)) as noutmny, ");
      strSql.append(" sum(coalesce(m.noutplanedmny, 0)) as noutplanedmny, sum(coalesce(m.noutvarymny, 0)) as noutvarymny ");

      if (query.isShowAssistant()) {
        strSql.append(", sum(coalesce(m.noutassistnum, 0)) as noutassistnum ");
      }

      if (query.isShowAdjustMoney()) {
        strSql.append(", sum(case when m.cbilltypecode='I9' then coalesce(m.noutmny, 0) else 0 end) as noutadjustmny ");
      }

      if (query.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)) {
        strSql.append(", sum(case when m.bwithdrawalflag='Y' then coalesce(m.noutnum, 0) else 0 end ) as njtlnum ");
      }
      ...
      sHaving = sHaving + " sum(coalesce(m.noutnum, 0))!=0 or sum(coalesce(m.noutmny, 0))!=0 or sum(coalesce(m.noutplanedmny, 0))!=0 ";
    }
    ...
    strSql.append(" and m.cbilltypecode in ('I5','I6','I7','I8','IA','IC','IJ') ");
    if (((query.getWhere() != null) && (query.getWhere().length() > 0)) || ((query.getDataPowerSql() != null) && (query.getDataPowerSql().length() > 0)))
    {
      String sWhere = query.getWhere();
      if ((sWhere.length() > 0) && (query.getDataPowerSql() != null) && (query.getDataPowerSql().trim().length() > 0))
    ...
    throws BusinessException
  {
    ...
    for (int i = 0; i < corps.length; ++i) {
      String sPeriod = cbo.getPeriod(corps[i], beginDate);
      String ePeriod = cbo.getPeriod(corps[i], endDate);
      ...
      else if ((endPeriod.compareToIgnoreCase(lastAccPeriod) > 0) && (startPeriod.compareTo(lastAccPeriod) <= 0))
      {
        ...
        if (startPeriod.compareTo(lastAccPeriod) <= 0)
          continue;
        if (strForBill.length() > 0) {
          strForBill.append(" or ");
        }
  ...
  private void exceLine(QueryVO queryVO, String tempTable, String[] sCorpAndPeriod)
    throws BusinessException
  {
    CommonDataImpl cbo = new CommonDataImpl();
    StringBuffer strSql = new StringBuffer();
    ...
    if ((sCorpAndPeriod[0] != null) && (sCorpAndPeriod[0].length() != 0))
    {
      ...
      strSql.append(", noutnum,noutmny, noutplanedmny,noutvarymny ");
      if (queryVO.isShowAssistant()) {
        strSql.append(", noutassistnum ");
      }
      if (queryVO.isShowAdjustMoney()) {
        strSql.append(", noutadjustmny ");
      }
      if (queryVO.SourceTable.equalsIgnoreCase(QueryVO.OUTBILL)) {
        strSql.append(", njtlnum ");
      }
