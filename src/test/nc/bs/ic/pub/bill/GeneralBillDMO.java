...
* 创建日期：(2001-6-12)
* @author：王乃军
  2002-03-29  修改copyItem等方法的为包可访问属性(private --> default)。支持业务代码转移到BO
*/
public class GeneralBillDMO
  extends DataManageObject
  implements nc.bs.pub.pf.IQueryData,nc.bs.pub.pf.IQueryData2 {
   ...
   */
  public GeneralBillDMO() throws javax.naming.NamingException, SystemException {
    super();
  }
   ...
   * @exception nc.bs.pub.SystemException 父类构造子抛出的异常。
   */
  public GeneralBillDMO(String dbName)
...
/**
   ...
   * @param sWhID java.lang.String
   * @param sInvID java.lang.String
   */
public void cancelInSerialsForCopy(
  GeneralBillHeaderVO voHeader,
...
//          sTempSN = voTempSN.getVserialcode();
          ...
          if (iSNstatus == SerialVO.NOTEXIST && vNotEx != null) {
            ...
            if (iSNstatus == SerialVO.OUTWH) {
              if (sBodyPK.equals(sSNbodyPK)) {
                ...
                iCorrectCount++;
                //nc.vo.scm.pub.SCMEnv.out("---> don't check out!");
              }
  ...
  //都正确：
  nc.vo.scm.pub.SCMEnv.out("---> ok number is " + iCorrectCount + "/" + alRet.size());
  if (alRet != null && iCorrectCount == alRet.size())
 ...
 * 返回：
 * 例外：
 * 日期：(2001-6-14 9:13:17)
 * 修改日期，修改人，修改原因，注释标志：
 * @return boolean
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public Boolean cancelSign(ArrayList alParam)
...
//   保留的系统管理接口：
  ...
  if (alParam != null && alParam.size() >= 4) {
    ...
    //检查通过把操作员ID置成NULL
    if (cancelSignCheck(sBillPK, sRegisterID, sCorpID, sCurDate, sCurBillType)) {
      String sql =
        "UPDATE ic_general_h SET  cregister = NULL, daccountdate = NULL,fbillflag=?, taccounttime = NULL  WHERE cgeneralhid = ?";
    ...
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000184")/*@res "取消签字检查参数错误！"*/);
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public Boolean cancelSign(GeneralBillVO voBill)
  throws BusinessException, Exception {
//   保留的系统管理接口：
  beforeCallMethod(
  "nc.bs.ic.pub.bill.GeneralBillDMO",
  ...
  new Object[] {voBill});
   ...
   //v5: 取消签字
  GeneralBillBOHelper.modifyBillsSign(new GeneralBillVO[]{voBill}, false);
  ...
  if (voHead.getDaccountdate() != null)
    ...
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000163")/*@res "库房取消签字时发现非法的数据：没有取消签字日期。"*/);

    //已经审核，不能取消签字
  if(voHead.getCauditorid()!=null&&voHead.getCauditorid().trim().length()>0)
      throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000185")/*@res "单据已经存货记账，不能取消签字！"*/);

  //  调用签字
  Boolean bret=cancelSign(sBillPK, sRegisterID, sCorpID, sCurDate, sCurBillType);
//   保留的系统管理接口：
  afterCallMethod(
  "nc.bs.ic.pub.bill.GeneralBillDMO",
  "cancelSign",
...
private boolean isSearchBarCodeOpen(ConditionVO[] voaCond) {
  ...
  for (int i=0;i<voaCond.length;i++){
    ...
    if (voaCond[i].getFieldCode().equalsIgnoreCase("body.bbarcodeclose")){
...
/**
   ...
   * 且cancelSignCheck()方法的参数已经修改，故暂时引掉。
   * 邵兵　on Jun 6, 2005
   */
/***
public boolean cancelSignCheck(ArrayList alParam)
  throws BusinessException, Exception {
  // 保留的系统管理接口：
  beforeCallMethod(
    "nc.bs.ic.pub.bill.GeneralBillDMO",
  ...
  if (alParam == null
    ...
    || alParam.get(0) == null
    || alParam.get(1) == null) {
    nc.vo.scm.pub.SCMEnv.out("sign check param err.");
    return false;
  }
  //单据PK
  String sBillPK = alParam.get(0).toString();
  //操作员
  String sRegisterID = alParam.get(1).toString();
  //执行
  return cancelSignCheck(sBillPK,sRegisterID,null,null);
}
***/
...
public boolean cancelSignCheck(GeneralBillVO voBill)
  ...
  if (voHead.getDaccountdate() != null)
    ...
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000163")/*@res "库房取消签字时发现非法的数据：没有取消签字日期。"*/);
  //调用签字
  return cancelSignCheck(sBillPK, sRegisterID, sCorpID, sCurDate, sCurBillType);
}    
...
/**
   ...
   * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
   * 
   * 采用ForwardBillDMO.isHaveForwardBill(String sCurBillType,String sCurBillPK)判断有无下游单据。
   * 修改该方法，多传递一个参数：单据类型。
   * 邵兵 on Jun 6, 2005
   * 不建议这样传递参数：方法不便于扩展。
   */
public boolean cancelSignCheck(
  String sBillPK,
  String sRegisterID,
  String sCorpID,
  String sCurDate,
...
//    return true;
  ...
  if (sBillStatus != null) {
    if (sBillStatus.equals(BillStatus.AUDITED))
      sbErrMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000351")/*@res "该单据已存货记账，不能取消签字。"*/+"\n");
    else if (sBillStatus.equals(BillStatus.DELETED))
      throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000186")/*@res "此单据已被删除。"*/);
    else if (sBillStatus.equals(BillStatus.FREE))
      sbErrMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000352")/*@res "该单据当前未签字。"*/+"\n");
  } else
    sbErrMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000353")/*@res "此单据状态未知。"*/);

  Boolean bRet=isDmSigned(sCorpID,sBillPK);
  if (bRet != null && bRet.booleanValue()==true){
    bOK=bRet.booleanValue();
    sbErrMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000488")/*@res "该单据已经做过发运签收或者结算，不能取消签字！"*/+"\n");
  }
  ...
  if (bHaveDownBill){
    bOK = bHaveDownBill;
    sbErrMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000489")/*@res "该单据已经生成下游单据，不能取消签字！"*/+"\n");
  }
  ...
  ArrayList alRet = cancelSignCheckChldBill(alTempParam);
  if (alRet == null) //something ERROR.
    sbErrMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000355")/*@res "其它错误:子单据是否已经签字未知。"*/+"\n");
  else if (alRet.size() == 0) //OK
    bOK = true;
  else {
    StringBuffer sbMsg = new StringBuffer(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000187")/*@res "以下子单据："*/);
    ...
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000188")/*@res "已经签字。请先取消签字这些单据。"*/);
...
public ArrayList cancelSignCheckChldBill(ArrayList alParam)
  ...
  //读存货数据
  if (sBillPK == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000189")/*@res "取消签字检查参数错误！无单据PK。"*/);

  StringBuffer sbSql = new StringBuffer();
  //sbSql.append(" SELECT DISTINCT h.cgeneralhid,h.pk_corp,h.cbilltypecode,bt.billtypename AS cbilltypename, h.vbillcode,h.dbilldate,cregister,h.daccountdate,cauditorid,h.dauditdate \n");
  sbSql.append(
    " SELECT DISTINCT bt.billtypename AS cbilltypename, h.vbillcode \n");

  sbSql.append(
    " FROM ic_general_h h INNER JOIN ic_general_b b ON h.cgeneralhid=b.cgeneralhid \n");
  sbSql.append(
    " LEFT OUTER JOIN bd_billtype bt ON bt.pk_billtypecode=h.cbilltypecode \n");
  sbSql.append(" WHERE h.dr=0 and b.csourcebillhid='");
  sbSql.append(sBillPK.trim());
  sbSql.append(
    "' AND (cregister IS NOT NULL AND daccountdate IS NOT NULL OR cauditorid IS NOT NULL AND dauditdate IS NOT NULL)  ");
 ...
 * 此处插入方法说明。
 * 创建日期：(2001-7-5 14:22:41)
 * @param vo nc.vo.ic.pub.bill.GeneralBillVO
 * @exception java.rmi.RemoteException 异常说明。
 */
public void check(GeneralBillVO vo)
  throws nc.vo.pub.BusinessException, Exception {
...
public void copyBill(GeneralBillVO voBill) throws BusinessException {
  ...
  try {
    ...
    //  复制表头：
    GeneralBillHeaderVO hvo =
      copyHeader((GeneralBillHeaderVO) voBill.getParentVO());
    ...
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000190")/*@res "复制单据错误："*/ + e.getMessage());
  }
  /*************************************************************/
  // 保留的系统管理接口：
  afterCallMethod(
 ...
 *
 * 创建日期：(2001-5-14)
 * @param node nc.vo.ic.ic201.GeneralHHeaderVO
...
void copyItembb1(GeneralBillItemVO[] voItems, String sCorpID)
  ...
  String sql =
    "insert into ic_general_bb1(cgeneralbb1,cgeneralbid, cspaceid,ninspacenum, ninspaceassistnum, noutspacenum,noutspaceassistnum,dr) values(?, ?, ?, ?, ?, ?, ?,1)";
    ...
    for (int j = 0; j < alVOs.size(); j++) {
      ...
      for (int i = 0; i < svoLocators.length; i++) {
        ...
        if (vo.getNinspacenum() == null) {
          ...
          stmt.setBigDecimal(4, vo.getNinspacenum().toBigDecimal());
        }

        if (vo.getNinspaceassistnum() == null) {
          stmt.setNull(5, Types.INTEGER);
        } else {
          stmt.setBigDecimal(5, vo.getNinspaceassistnum().toBigDecimal());
        }
        if (vo.getNoutspacenum() == null) {
          stmt.setNull(6, Types.INTEGER);
        } else {
          stmt.setBigDecimal(6, vo.getNoutspacenum().toBigDecimal());
        }

        if (vo.getNoutspaceassistnum() == null) {
          stmt.setNull(7, Types.INTEGER);
        } else {
          stmt.setBigDecimal(7, vo.getNoutspaceassistnum().toBigDecimal());
        }
        executeUpdate(stmt);
...
String copyItembb3(
  ...
  String sql =
    //"insert into ic_general_bb3 (cgeneralbb3, cgeneralhid, cgeneralbid) values(?, ?, ? ) ";
  ...
  ArrayList alVOs = new ArrayList();
  for (int i = 0; i < voItems.length; i++) {
    nc.vo.ic.pub.settlement.SettlementVO[] votmpSets = voItems[i].getBB3VO();
  ...
  try {
    ...
    for (int k = 0; k < alVOs.size(); k++) {
      ArrayList al = (ArrayList) alVOs.get(k);
      nc.vo.ic.pub.settlement.SettlementVO[] voSets =
        (nc.vo.ic.pub.settlement.SettlementVO[]) al.get(0);
      String cgeneralbid = (String) al.get(1);
      for (int i = 0; i < voSets.length; i++) {
        ...
        if (settlement.getNsettlemny1() == null) {
          ...
          stmt.setBigDecimal(4, settlement.getNsettlemny1().toBigDecimal());
        }
        if (settlement.getNaccountnum2() == null) {
          stmt.setNull(5, Types.INTEGER);
        } else {
          stmt.setBigDecimal(5, settlement.getNaccountnum2().toBigDecimal());
        }
        ...
        if (settlement.getNsettlenum1() == null) {
          ...
          stmt.setBigDecimal(7, settlement.getNsettlenum1().toBigDecimal());
        }
        if (settlement.getNsignnum() == null) {
          stmt.setNull(8, Types.INTEGER);
        } else {
          stmt.setBigDecimal(8, settlement.getNsignnum().toBigDecimal());
        }
        if (settlement.getNpprice() == null) {
          stmt.setNull(9, Types.INTEGER);
        } else {
          stmt.setBigDecimal(9, settlement.getNpprice().toBigDecimal());
        }
        if (settlement.getNpmoney() == null) {
          stmt.setNull(10, Types.INTEGER);
        } else {
          stmt.setBigDecimal(10, settlement.getNpmoney().toBigDecimal());
        }
        if (settlement.getNmaterialmoney() == null) {
          stmt.setNull(11, Types.INTEGER);
        } else {
          stmt.setBigDecimal(11, settlement.getNmaterialmoney().toBigDecimal());
        }
        //if (settlement.getNaccountassistnum2() == null) {
        //stmt.setNull(14, Types.INTEGER);
        //} else {
        //stmt.setBigDecimal(14, settlement.getNaccountassistnum2().toBigDecimal());
        //}
        //if (settlement.getNaccountassistnum1() == null) {
        //stmt.setNull(15, Types.INTEGER);
        //} else {
        //stmt.setBigDecimal(15, settlement.getNaccountassistnum1().toBigDecimal());
        //}
        //if (settlement.getNsignassistnum() == null) {
        //stmt.setNull(16, Types.INTEGER);
        //} else {
        //stmt.setBigDecimal(16, settlement.getNsignassistnum().toBigDecimal());
        //}
        if (settlement.getNdmsignnum() == null) {
          stmt.setNull(12, Types.INTEGER);
        } else {
          stmt.setBigDecimal(12, settlement.getNdmsignnum().toBigDecimal());
        }
        if (settlement.getNrsvnum1() == null) {
          stmt.setNull(13, Types.INTEGER);
        } else {
          stmt.setBigDecimal(13, settlement.getNrsvnum1().toBigDecimal());
        }
        if (settlement.getNrsvnum2() == null) {
          stmt.setNull(14, Types.INTEGER);
        } else {
          stmt.setBigDecimal(14, settlement.getNrsvnum2().toBigDecimal());
        }
...
/**
   ...
   * @param voBill nc.voDeleteBill.ic.pub.bill.GeneralBillVO
   */
public final void deleteBill(String sBillPK)
  throws java.sql.SQLException, BusinessException, Exception {
  /*************************************************************/
  // 保留的系统管理接口：
  beforeCallMethod(
    "nc.bs.ic.pub.bill.GeneralBillDMO",
  ...
  /*************************************************************/
  if (sBillPK == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000162")/*@res "VO参数错误。"*/);
  ...
  if (alBill == null || alBill.size() == 0)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000191")/*@res "未查到当前的单据，可能已被删除。"*/);

  //读出的单据
  GeneralBillVO voBill = (GeneralBillVO) alBill.get(0);
  if (voBill == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000192")/*@res "查询的结果错误。"*/);

  GeneralBillHeaderVO voHeader = voBill.getHeaderVO();
  //检查是否已经签字了
  if (voHeader.getCregister() != null
    && voHeader.getCregister().toString().trim().length() > 0)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000193")/*@res "此单据已签字！"*/);
  ...
  if (voaItem == null || voaItem.length == 0)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000194")/*@res "删除查询的结果表体错误。"*/);
...
//  alChildrenBill = queryRelatedBillInfo(sBillPK);
  ...
  //如果有错，抛异常
  checkBillInfo(alChildrenBill, nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000195")/*@res "本单据有如下相关单据:\n"*/, nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000196")/*@res "\n不能删除。"*/);
  ...
  //bb3
  nc.bs.ic.pub.settlement.SettlementDMO stdmo =
    new nc.bs.ic.pub.settlement.SettlementDMO();
  //nc.bs.ic.pub.sn.SerialDMO sdmo = new nc.bs.ic.pub.sn.SerialDMO();
  //记录序列号状态
  //存在
  ...
  for (int row = 0; row < voaItem.length; row++) {
    if (voaItem[row] != null) {
      ...
      if (voasn != null && voasn.length > 0) {
        voaItem[row].setSerial(voasn);
        cancelOutSerials(voBill.getHeaderVO(), voaItem[row], vEx, vNotEx, vRet, vOut);
      }
      //---- 09 Oct ,wnj
    }
  }
...
/**
 * 采购调用的删除推式生成的采购入库单的方法。
 * 功能：
 * 参数：
 * 返回：
 * 例外：
 * 日期：(2001-10-18 12:44:36)
 * 修改日期，修改人，修改原因，注释标志：
 */
public void deleteBySourceID(String sourceHeadID[], String sSourceType) throws BusinessException, Exception {

  /*************************************************************/
  // 保留的系统管理接口：
  beforeCallMethod("nc.bs.ic.pub.bill.GeneralBillDMO", "deleteBySourceID", new Object[] { sourceHeadID,sSourceType });
  /*************************************************************/
  if (sourceHeadID == null || sSourceType == null) {
    nc.vo.scm.pub.SCMEnv.out("can not find the sourcehid from the parameter");
    return;
  }
  StringBuffer sbSql2 = new StringBuffer("select cgeneralhid from ic_general_b where dr=0  AND ");
  StringBuffer sbWhere = new StringBuffer("  csourcetype=? "); //AND csourcebillhid IN (");
  //使用临时表
  sbWhere.append(nc.bs.ic.pub.bill.GeneralSqlString.formInSQL("csourcebillhid", sourceHeadID));
  //循环写入条件
  ...
  try {
    ...
    rs = stmt.executeQuery();
    if (rs.next())
      throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000197")/*@res "请先删除相关的库存单据，然后再试。"*/);
  } finally {
    try {
      if (rs != null)
        rs.close();
  ...
  /*************************************************************/
  // 保留的系统管理接口：
  afterCallMethod("nc.bs.ic.pub.bill.GeneralBillDMO", "deleteBySourceID", new Object[] { sourceHeadID,sSourceType });
  /*************************************************************/
}
 ...
 * 修改日期，修改人，修改原因，注释标志：
 */
public void deleteBySourceID_backup(String sourceHeadID[], String sSourceType)
  throws BusinessException, Exception {

  /*************************************************************/
  // 保留的系统管理接口：
  beforeCallMethod(
      "nc.bs.ic.pub.bill.GeneralBillDMO", "deleteBySourceID_backup", new Object[] { sourceHeadID,sSourceType });
  /*************************************************************/
  if (sourceHeadID == null || sSourceType == null) {
    nc.vo.scm.pub.SCMEnv.out("can not find the sourcehid from the parameter");
    return;
  ...
  StringBuffer sbSql =
    new StringBuffer("update ic_general_b  set dr=1 where dr=0 AND (");
  StringBuffer sbSql2 =
    new StringBuffer("update ic_general_h set dr=1 where dr=0 AND cgeneralhid IN (select cgeneralhid from ic_general_b where dr=1  AND ");
  StringBuffer sbWhere =
    new StringBuffer("  csourcetype=? ");// AND csourcebillhid IN (");
  ...
  try {
    ...
    stmt.executeUpdate();
    //--------2--------
    stmt = con.prepareStatement(sbSql2.append(sbWhere).append(")").toString());
    stmt.setString(1, sSourceType);
    stmt.executeUpdate();
  } finally {
    try {
...
protected ArrayList deleteChildrenBillXXX(String sParentBillPK)
  ...
  beforeCallMethod(
    "nc.bs.ic.pub.bill.GeneralBillDMO",
    "cancelSignCheckChldBillXXX",
    new Object[] { sParentBillPK });
  /*************************************************************/
  ...
  if (alChildrenBill == null)
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000198")/*@res "子单据其它错误。\n"*/);
  else if (alChildrenBill != null && alChildrenBill.size() > 0) {
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000199")/*@res "子单据"*/);
    //zhx I18N
    ...
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000200")/*@res "已经签字，请取消签字后再试！\n"*/);
  }
 ...
 *  修改日期，修改人，修改原因，注释标志：
 */
protected GeneralBillItemVO[] filterUselessUOM(GeneralBillItemVO[] voaBillItem) {
...
/**
 * <p>根据表头的主键查询属于本单据的表体pk。用于删除时
 * <p>
 * 创建日期：(2001-5-14)
 * @param key ??dbFieldType??
 * @exception java.sql.SQLException 异常说明。
 */
protected Vector findAllBodyPK(String sBillPK) throws Exception {

  Vector vPK = new Vector();
  ;
  String sql =
    "select   cgeneralbid from ic_general_b where dr=0 AND cgeneralhid ='"
      + sBillPK
      + "' order by cgeneralbid ";
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public String getBillStatus(String sBillPK) throws Exception {
  /*************************************************************/
  // 保留的系统管理接口：
  beforeCallMethod(
    "nc.bs.ic.pub.bill.GeneralBillDMO",
  ...
  //读存货数据
  if (sBillPK == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000201")/*@res "检查单据是否签字时未传入单据ID."*/);
  //缺省未签字，。
  String sRet = BillStatus.FREE;
  String sSql =
    " SELECT cregister,cauditorid FROM ic_general_h WHERE dr=0 AND cgeneralhid=?";
    ...
    if (rs.next()) {
      ...
      if ((cregister == null || cregister.trim().length() == 0)
        ...
        cregister != null
          && cregister.trim().length() > 0
          && cauditorid != null
          && cauditorid.trim().length() > 0)
        sRet = BillStatus.AUDITED;
      nc.vo.scm.pub.SCMEnv.out("reg=" + cregister + " ---> " + cauditorid);
...
/**
 * 此处插入方法说明。
 * 创建日期：(2003-10-14 16:04:32)
 * @return ArrayList
        String[0] hid;
        String[1] code
 * @param
 */
private ArrayList getFirstBillCodes(String billtype,String subQry) throws Exception{
  if(billtype==null||subQry==null)
    return null;

  //String sWhere=" in (select cfirstbillhid from ic_general_b where cgeneralhid='"+cgeneralhid+"')";
  ...
  if(billtype.equalsIgnoreCase("30")||billtype.equalsIgnoreCase("3A")){
    ...
    s.append(" csaleid,vreceiptcode from so_salereceipt where csaleid ");

    }
  else if (billtype.equalsIgnoreCase("32")||billtype.equalsIgnoreCase("3C")){
    s.append(" csaleid,vreceiptcode from so_saleinvoice where csaleid ");
    ...
    s.append(" carriveorderid,varrordercode from po_arriveorder where carriveorderid ");
    ...
    s.append(" cgeneralhid,vbillcode from ic_general_h where cgeneralhid ");
    ...
    s.append(" cspecialhid,vbillcode from ic_special_h where cspecialhid ");

      }
  else  if (billtype.equalsIgnoreCase("4U")){
    s.append(" callocationhid,vbillcode from ic_allocation_h where callocationhid ");
    }
 ...
 *  修改日期，修改人，修改原因，注释标志：
 */
protected ArrayList getFreeItemData(ArrayList alResultData) throws Exception {
  //查自由项
  if (alResultData == null || alResultData.size() == 0)
    return alResultData;
  ...
  ////读自由项
  //nc.bs.ic.pub.freeitem.DefdefDMO defdmo = new nc.bs.ic.pub.freeitem.DefdefDMO();
  ...
  //for (int bill = 0; bill < alResultData.size(); bill++) {
    ...
    //for (int row = 0; row < voaTempBillItem.length; row++) {
      ...
      //if (htid.containsKey(voaTempBillItem[row].getCinventoryid())) {
        //voBasFree =
          //(FreeVO) ((FreeVO) htFree.get(voaTempBillItem[row].getCinventoryid())).clone();
      //}
...
//    查自由项
  ...
  //for (int row = 0; row < voaItem.length; row++) {
    ...
    //if (htFree.containsKey(voaItem[row].getCinventoryid())) {
      //voBasFree =
        //(FreeVO) ((FreeVO) htFree.get(voaItem[row].getCinventoryid())).clone();
 ...
 *  修改日期，修改人，修改原因，注释标志：
 */
protected GeneralBillItemVO[] getFreeItemData_OLD(GeneralBillItemVO[] voaItem)
  throws Exception {
  //查自由项
  if (voaItem == null || voaItem.length == 0)
    return voaItem;
 ...
 * 创建者：王乃军
 * 功能：得到序列号提示信息
 * 参数：Vector vEx, Vector vNotEx, Vector vRet, Vector vOut
     存在       不存在     返回        已出库
 * 返回：
 * 例外：
 * 日期：(2001-10-18 11:46:08)
...
public String getSnHintMsg(
  ...
  if (iCount > 0) {
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000167")/*@res "重复的序列号："*/);
    for (int sn = 0; sn < iCount; sn++) {
      ...
      if (sn != iCount - 1)
        sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPPSCMCommon-000000")/*@res "、"*/);
      else
        sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000168")/*@res "。"*/);
    }

  }
  iCount = vNotEx.size();
  if (iCount > 0) {
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000345")/*@res "不存在的序列号："*/);
    for (int sn = 0; sn < iCount; sn++) {
  ...
  iCount = vRet.size();
  if (iCount > 0) {
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000169")/*@res "已退回的序列号："*/);
    for (int sn = 0; sn < iCount; sn++) {
  ...
  iCount = vOut.size();
  if (iCount > 0) {
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000202")/*@res "已出库的序列号："*/);
    for (int sn = 0; sn < iCount; sn++) {
...
public String getSnHintMsg(
  ...
  Vector vFreeze) {
  //-------------
  StringBuffer sbMsg = new StringBuffer(getSnHintMsg(vEx, vNotEx, vRet, vOut));
  //判断序列号的状态，出现在返回结果中的序列号都是有问题的。
  nc.vo.ic.pub.sn.SerialVO voSn = null;
  int iCount = 0;
  iCount = vFreeze.size();
  if (iCount > 0) {
    sbMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000203")/*@res "已冻结的序列号："*/);
    for (int sn = 0; sn < iCount; sn++) {
 ...
 * @param
 */
private ArrayList getSourceRowNos(String billtype,String shid) throws Exception {
  if(billtype==null)
    return null;

  m_timer.start();

  String subQry=" select csourcebillbid from ic_general_b where csourcetype='"+billtype+"'" ;
  ...
  StringBuffer s=new StringBuffer("select ");
  if(billtype.equalsIgnoreCase("30")||billtype.equalsIgnoreCase("3A")){
    s.append(" corder_bid,crowno from so_saleorder_b where corder_bid ");
  }

  else if (billtype.equalsIgnoreCase("32")||billtype.equalsIgnoreCase("3C")){
    s.append(" cinvoice_bid,crowno from so_saleinvoice_b where cinvoice_bid ");
    ...
    s.append(" carriveorder_bid,crowno from po_arriveorder_b where carriveorder_bid ");
  ...
  //else if (billtype.equalsIgnoreCase("A4")){
    ...
    s.append(" cgeneralbid,crowno from ic_general_b where cgeneralbid ");
    ...
    ||billtype.equalsIgnoreCase("4N")
    ||billtype.equalsIgnoreCase("4R")){
    s.append(" cspecialbid,crowno from ic_special_b where cspecialbid ");

      }
  else  if (billtype.equalsIgnoreCase("4U")){
    s.append(" callocationbid,crowno from ic_allocation_b where callocationbid ");
    }
  else  if (billtype.equals("7F")){
    s.append(" pk_delivbill_b,irownumber from dm_delivbill_b where pk_delivbill_b ");
    }
  else
    return null;
  ...
  m_timer.stopAndShow("@@:--)查询来源行号："+billtype+String.valueOf(alhidcode.size()));
  return alhidcode;
}
...
public String getSourceRowNoSQL(String billtype) throws Exception {
  ...
  //"FROM so_saleorder_b b WHERE ic_general_b.csourcebillbid = b.corder_bid AND ic_general_b.csourcetype = '30'"
  ...
  if(billtype.equals("7D"))
    return
    "update ic_general_b set vsourcerowno='1' where dr=0 and  csourcetype='7D' and vsourcerowno is null ";

  //发运单
  if(billtype.equals("7F"))
    return
  " update ic_general_b set vsourcerowno=b.irownumber from dm_delivbill_b b  where ic_general_b.csourcebillbid=b.pk_delivbill_b and ic_general_b.csourcetype='7F' and ic_general_b.dr=0 and ic_general_b.vsourcerowno is null";


  StringBuffer s=new StringBuffer("update ic_general_b set vsourcerowno=b.crowno");
  if(billtype.equalsIgnoreCase("30")||billtype.equalsIgnoreCase("3A")){
    s.append(" from so_saleorder_b b WHERE ic_general_b.csourcebillbid = b.corder_bid ");
  }

  else if (billtype.equalsIgnoreCase("32")||billtype.equalsIgnoreCase("3C")){
    s.append("  from so_saleinvoice_b b where ic_general_b.csourcebillbid = b.cinvoice_bid ");

    }
  else if (billtype.equalsIgnoreCase("3U")||billtype.equalsIgnoreCase("3V")){
    s.append(" from so_apply_b b where ic_general_b.csourcebillbid = b.pk_apply_b ");

    }
  else if (billtype.equalsIgnoreCase("21")){
    s.append(" from po_order_b b where  ic_general_b.csourcebillbid = b.corder_bid ");

    }
  else if (billtype.equalsIgnoreCase("23")){
    s.append("  from po_arriveorder_b b where ic_general_b.csourcebillbid = b.carriveorder_bid ");

    }
  else if (billtype.equalsIgnoreCase("61")){
    s.append("   from sc_order_b  b where ic_general_b.csourcebillbid = b.corder_bid ");

    }
  else if (billtype.startsWith("5")){
    s.append("   from to_bill_b  b where ic_general_b.csourcebillbid = b.cbill_bid ");
  ...
  //else if (billtype.equalsIgnoreCase("41")
    ...
    //||billtype.equalsIgnoreCase("4C")
    //||billtype.equalsIgnoreCase("45")){
    //s.append("   from ic_general_b b where ic_general_b.csourcebillbid = b.cgeneralbid ");
    ...
    ||billtype.equalsIgnoreCase("4N")
    ||billtype.equalsIgnoreCase("4R")){
    s.append("  from ic_special_b  b where ic_general_b.csourcebillbid = b.cspecialbid ");

      }
  else  if (billtype.equalsIgnoreCase("4U")){
    s.append("  from ic_allocation_b  b where ic_general_b.csourcebillbid = b.callocationbid ");
    }
  ...
  s.append(" AND ic_general_b.csourcetype = '").append(billtype).append("'")
  .append(" and ic_general_b.dr=0 and ic_general_b.vsourcerowno is null ");
...
public String getSourceRowNoWhereString(String billtype) throws Exception {
  ...
  else  if (billtype.equalsIgnoreCase("4U")||billtype.equalsIgnoreCase("5E")){
    s.append("  from ic_allocation_b  b where ic_general_b.csourcebillbid = b.callocationbid ");
    }
...
public ArrayList getTs(String sBillPK) throws SQLException, Exception {
  ...
  /*************************************************************/
  String sSql1 =
    "SELECT h.ts,b.cgeneralbid,b.ts AS bts,ninnum,ninassistnum,noutnum,noutassistnum,vfirstbillcode,vbatchcode,nbarcodenum,bbarcodeclose,b.nprice,b.nmny FROM ic_general_b b INNER JOIN       ic_general_h h ON b.cgeneralhid = h.cgeneralhid WHERE h.cgeneralhid=? and b.cgeneralhid=? and  h.dr=0 AND b.dr=0 ";
  //返回值
  ArrayList alRet = new ArrayList();
 ...
 * 返回：
 * 例外：
 * 日期：(2002-4-1 20:06:45)
 * 修改日期，修改人，修改原因，注释标志：
 * @return nc.vo.pub.AggregatedValueObject
 * @param voCur nc.vo.pub.AggregatedValueObject
 * @param voPre nc.vo.pub.AggregatedValueObject
 */
public GeneralBillVO[] getVOSForUpdate2(AggregatedValueObject voCur)
  ...
  GeneralBillHeaderVO voCurHead = (GeneralBillHeaderVO) checkVO.getHeaderVO();
  GeneralBillItemVO[] voCurItems = (GeneralBillItemVO[]) checkVO.getItemVOs();
  ...
  try {
    Hashtable ht = null;
    if (whvo.getIsLocatorMgt()==null||whvo.getIsLocatorMgt().intValue()!=1) {

      ht = queryNotAllZero(checkVO);//(cgeneralhid, cwarehouseid); //仓库数量倒挤
...
//            continue;
          ...
          if (voItems[i].getCastunitid() == null||voItems[i].getHsl()==null||voItems[i].getHsl().doubleValue()==0.0
            || voItems[i].getStatus() == VOStatus.DELETED
            || (voItems[i].getFchecked() != null
              && voItems[i].getFchecked().intValue() != 0)
            ||(voItems[i].getSerial()!=null&&voItems[i].getSerial().length>0)
          )
            continue;
          key = new StringBuffer();
          key
            .append(voItems[i].getCinventoryid())
            .append(voItems[i].getVfree1())
            .append(voItems[i].getVfree2())
            .append(voItems[i].getVfree3())
            .append(voItems[i].getVfree4())
            .append(voItems[i].getVfree5())
            .append(voItems[i].getVfree6())
            .append(voItems[i].getVfree7())
            .append(voItems[i].getVfree8())
            .append(voItems[i].getVfree9())
            .append(voItems[i].getVfree10())
            .append(voItems[i].getVbatchcode())
            .append(voItems[i].getCastunitid());
          ...
          if (ht.containsKey(key.toString())) {
            voChangedItem = (GeneralBillItemVO) ht.get(key.toString());
            if((voChangedItem.getNinnum()==null
                  ||voChangedItem.getNinnum().compareTo(ZERO)==0)
                &&(voChangedItem.getNinassistnum()==null
                  ||voChangedItem.getNinassistnum().compareTo(ZERO)==0)){
            ...
            nc.vo.scm.pub.SCMEnv.out("发生倒挤：" + voNewItem.getCinventorycode());
      ...
      ht = queryNotAllZeroCargo(checkVO);//(cgeneralhid, cwarehouseid);
      if (ht != null && ht.size() > 0) {
        StringBuffer key = null;
...
//        UFDouble assistnum = null;
          ...
          if (voItems[i].getLocator() != null) {
            ...
            for (int j = 0; j < voLocs.length; j++) {
              ...
              if (ht.containsKey(key1.toString())) {
                voChangedItem = (GeneralBillItemVO) ht.get(key1.toString());
                if((voChangedItem.getNinnum()==null
                  ||voChangedItem.getNinnum().compareTo(ZERO)==0)
                &&(voChangedItem.getNinassistnum()==null
                  ||voChangedItem.getNinassistnum().compareTo(ZERO)==0)){
                ...
                //vOld.addElement(voItems[i].clone());
                nc.vo.scm.pub.SCMEnv.out("发生倒挤：" + voNewItem.getCinventorycode());
                relation2(voItems[i],voChangedItem,voLocs[j]);
                isFound = true;
              }
            }
          }
    ...
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000176")/*@res "数量倒挤："*/ + e.getMessage());
  }
  GeneralBillVO voNewBill = null;
  if (vNew.size() > 0) {
    voNewBill = new GeneralBillVO();
    voNewBill.setParentVO((GeneralBillHeaderVO) checkVO.getParentVO().clone());
  ...
  if (vOld.size() > 0) {
    ...
    voOldBill.setParentVO((GeneralBillHeaderVO) checkVO.getParentVO().clone());
...
//          sTempSN = voTempSN.getVserialcode();
  ...
  nc.vo.scm.pub.SCMEnv.out("---> ok number is " + iCorrectCount + "/" + voaSn.length);
  //都正确-->入库
...
//  如果是供应商管理的需要记录供应商,不是供应商管理的不记录供应商
	if(voHeader.getIsgathersettle().booleanValue()||((voItem.getInv().getIssupplierstock()!=null) && "Y".equals(voItem.getInv().getIssupplierstock())))
...
//    voaSn[0].setCproviderid(voHeader.getCproviderid());
    ...
    for (int free = 1; free <= 10; free++)
      voaSn[0].setAttributeValue(
        "vfree" + free,
        voItem.getAttributeValue("vfree" + free));
...
* <p>
* 创建日期：(2001-5-14)
* @param voItem nc.vo.ic.ic201.GeneralHItemVO
* @param sBillPK String
* @exception java.sql.SQLException 异常说明。
*/
public String insertBillItem(
  GeneralBillItemVO voItem,
  String sBillPK,
  String sCorpID)
  throws BusinessException {
  if (voItem == null || sBillPK == null || sCorpID == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000204")/*@res "插入行错误：无单据PK."*/);
  try {
    voItem.setCgeneralhid(sBillPK);
    insertItemBatch(new GeneralBillItemVO[]{voItem});
    return voItem.getCgeneralbid();
  } catch (Exception e) {
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000205")/*@res "插入行错误："*/ + e.getMessage());
  }
 ...
 *
 * 创建日期：(2001-5-14)
 * @param node nc.vo.ic.ic201.GeneralHItemVO
 * @exception java.sql.SQLException 异常说明。
 */
...
//      nc.vo.scm.pub.SCMEnv.out("@@@@"+name[i]+" "+voItems[0].getAttributeValue(name[i]));
  SmartDMO dmo = null;
  try{
      dmo = new SmartDMO();
  }catch(NamingException e){
...
public Boolean isAudited(String sBillPK) throws Exception {
  ...
  StringBuffer sbSql =
    new StringBuffer(" SELECT cauditorid FROM ic_general_h WHERE dr=0 AND cgeneralhid='");
  sbSql.append(sBillPK.trim());
  sbSql.append("'");
...
public Boolean isDmSigned(String pk_corp,String sBillPK) throws Exception {
  ...
  nc.itf.uap.sf.ICreateCorpQueryService s = (nc.itf.uap.sf.ICreateCorpQueryService) NCLocator.getInstance().lookup("nc.itf.uap.sf.ICreateCorpQueryService");
...
//  boolean isPO=dmo.isEnabled(pk_corp,"PO");
  ...
  //naccountnum1
  StringBuffer sbSql =
    new StringBuffer(" SELECT sum(isnull(ndmsignnum,0)) as ndmsignnum,sum(isnull(ntoaccountnum,0.0)) as ntoaccountnum, sum(isnull(naccountnum1,0.0)) as naccountnum1, sum(isnull(nsettlenum1,0.0)) as nsettlenum1 FROM ic_general_bb3 WHERE dr=0 AND cgeneralhid=? ");
 ...
 * 功能：修改累计出库数量
 * 参数 :
    String[] sBillItemPK,  表体PK
    UFDouble[] dNum 数量
 * 返回：
 * 例外：
 * 日期：(2001-6-12 20:38:02)
 * 修改日期，修改人，修改原因，注释标志：
 * @return nc.vo.ic.pub.bill.GeneralBillVO[]
 * @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
 */
public void updateCorNumBatch(String[] sBillItemPKs, UFDouble[] dNums,UFDouble[] dAstNums,UFDouble[] dGrsNums)
  ...
  if (sBillItemPKs == null ||sBillItemPKs.length==0|| dNums == null || dNums.length==0)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000490")/*@res "传入参数不完整！"*/);


  String sql1 =
    "UPDATE ic_general_b SET  ncorrespondnum = COALESCE(ncorrespondnum,0.0)+(?), ncorrespondastnum = COALESCE(ncorrespondastnum,0.0)+(?),ncorrespondgrsnum = COALESCE(ncorrespondgrsnum,0.0)+(?)  WHERE cgeneralbid = ?";

  //String sql2 =
    //"UPDATE ic_general_b SET  ncorrespondastnum= case when  COALESCE(ninnum,abs(noutnum))- COALESCE(ncorrespondnum,0.0)=0 then COALESCE(ninassistnum,abs(noutassistnum)) else case when hsl = 0 then 0 else  COALESCE(ncorrespondnum,0.0)/ COALESCE(hsl,1.0) end end  WHERE cgeneralbid = ? and castunitid is not null ";
  ...
  try {
    ...
    for(int i=0;i<sBillItemPKs.length;i++){
      ...
      if(dAstNums!=null&&dAstNums.length>i&&dAstNums[i]!=null)
        stmt.setBigDecimal(2, dAstNums[i].toBigDecimal());
      else
        stmt.setBigDecimal(2, new BigDecimal(0.0));
      if(dGrsNums!=null&&dGrsNums.length>i&&dAstNums[i]!=null)
    ...
    if(sBillItemPKs.length>0){
      String ss=nc.bs.ic.pub.bill.GeneralSqlString.formInSQL("b.cgeneralbid",sBillItemPKs);
      StringBuffer sMsg=new StringBuffer();
      String sql=" select vbillcode ,crowno,num from ic_general_h h inner join (select b.cgeneralhid,b.crowno,abs(COALESCE(ninnum,noutnum))-abs(isnull(ncorrespondnum,0)) as num from ic_general_b b  where b.dr=0 "+ss+") tmp on h.cgeneralhid=tmp.cgeneralhid where num<0 ";
      stmt =con.prepareStatement(sql);
      ResultSet rs=stmt.executeQuery();
      while(rs.next()){
        sMsg.append("\n"+nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000491")/*@res "入库单据："*/+rs.getString(1));
        sMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000492")/*@res "；行号："*/+rs.getString(2));
        sMsg.append(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000493")/*@res "；超出："*/+rs.getBigDecimal(3).abs().toString());
      ...
      stmt =con.prepareStatement("select count(*) from ic_general_b b where b.dr=0 "+ss);
      rs=stmt.executeQuery();
      if(rs.next()){
        int count=rs.getInt(1);
        if(count!=sBillItemPKs.length){
          
          throw new BusinessException(ResBase.getCorInBillDelete());
        }
...
public Boolean isHaveEstimatedItems(String sBillPK)
  ...
  //单据PK
  if (sBillPK == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000206")/*@res "暂估检查参数错误！无单据PK。"*/);
  //返回值
  Boolean bHave = new Boolean(false);

  StringBuffer sbSql =
    new StringBuffer("SELECT cgeneralbid,bzgflag,isok FROM ic_general_b WHERE dr=0 AND cgeneralhid='");
  sbSql.append(sBillPK.trim());
  sbSql.append(
...
/**
   ...
   * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
   */
public Boolean isHaveSettledItems(String sBillPK)
  ...
  //单据PK
  if (sBillPK == null)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000207")/*@res "结算检查参数错误！无单据PK。"*/);
  //返回值
  Boolean bHave = new Boolean(false);

  String sSql =
    "SELECT cgeneralbid,isok FROM ic_general_b WHERE dr=0 AND cgeneralhid=?  AND  (rtrim(ltrim(COALESCE(isok,'N')))='Y' OR rtrim(ltrim(COALESCE(isok,'N')))='y' )";
 ...
 *  ******************************************
 * @return boolean
 * @param cbilltypecode java.lang.String
 */
private boolean isNeedBB3(String cbilltypecode)
{
  ...
  if (cbilltypecode != null
    && (cbilltypecode.equalsIgnoreCase(nc.vo.ic.pub.BillTypeConst.m_purchaseIn)
      ...
      || cbilltypecode.equalsIgnoreCase(nc.vo.ic.pub.BillTypeConst.m_consignMachiningOut)
      || cbilltypecode.equalsIgnoreCase(nc.vo.ic.pub.BillTypeConst.m_allocationIn)
      || cbilltypecode.equalsIgnoreCase(nc.vo.ic.pub.BillTypeConst.m_otherOut)
      || cbilltypecode.equalsIgnoreCase(nc.vo.ic.pub.BillTypeConst.m_allocationOut)))
    return true;
  else
    return false;
}
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public Boolean isSigned(String sBillPK)
  throws BusinessException, SQLException {
  /*************************************************************/
  // 保留的系统管理接口：
  beforeCallMethod(
  ...
  StringBuffer sbSql =
    new StringBuffer(" SELECT cregister FROM ic_general_h WHERE dr=0 AND cgeneralhid='");
  sbSql.append(sBillPK.trim());
  sbSql.append("'");
  ...
  try {
    ...
    if (rs.next()) {
      ...
      throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000208")/*@res "检查单据是否签字时找不到此单据。"*/);
...
/**
 * 创建者：张欣
 * 功能：根据传入的公司ID，来源单据Hid和来源单据类型，检查单据是否已签字（记账）。
 * 参数：String pk_corp, String sSourceBillHid, String sSourceBillType
 * 返回：true：已签字
     false：未签字
 * 例外：
 * 日期：(2001-6-14 9:11:16)
 * 修改日期，修改人，修改原因，注释标志：
 */
public Boolean isSourceBillSigned(
  String pk_corp,
  String sSourceBillHid,
  ...
  beforeCallMethod(
    "nc.bs.ic.pub.bill.GeneralBillDMO",
    "isSourceBillSigned",
    new Object[] { pk_corp, sSourceBillHid, sSourceBillType });
  /*************************************************************/
  Boolean bRet = null;
  //读存货数据
  if (pk_corp == null || sSourceBillHid == null || sSourceBillType == null) {
    nc.vo.scm.pub.SCMEnv.out("pk_corp or sourcebillhid or sourcebilltype is  null ERR.");
    return null;
  }
  StringBuffer sbSql =
    new StringBuffer(" SELECT h.cregister FROM ic_general_h h Inner Join ic_general_b b on(h.cgeneralhid =b.cgeneralhid and h.dr=0 and b.dr=0) WHERE h.pk_corp='");
  sbSql
    .append(pk_corp.trim())
    .append("' And b.csourcetype = '")
    .append(sSourceBillType)
    .append("' And b.csourcebillhid ='")
    .append(sSourceBillHid)
    .append("' ");
  Connection con = null;
  PreparedStatement stmt = null;
  ResultSet rs = null;
 ...
 * 修改日期，修改人，修改原因，注释标志：
 */
public Boolean isSourceBillsSigned(String[] params, String sSourceBillType) throws Exception {
  /*************************************************************/
  // 保留的系统管理接口：
  beforeCallMethod("nc.bs.ic.pub.bill.GeneralBillDMO", "isSourcesBillSigned", new Object[] { params, sSourceBillType });
  /*************************************************************/
  Boolean bRet = null;
  //读存货数据
  ...
  if (pk_corp == null || sSourceBillType == null) {
    nc.vo.scm.pub.SCMEnv.out("pk_corp or sourcebillbid or sourcebilltype is  null ERR.");
    return null;
  }
  StringBuffer sbSql =
    new StringBuffer(" SELECT h.cregister FROM ic_general_h h Inner Join ic_general_b b on(h.cgeneralhid =b.cgeneralhid and h.dr=0 and b.dr=0) WHERE h.pk_corp='");
  sbSql.append(pk_corp.trim()).append("'  AND h.cbilltypecode <> '4X' And b.csourcetype = '").append(sSourceBillType);

  sbSql.append("' ");
  //And b.csourcebillbid in (");
  //使用临时表
  sbSql.append(nc.bs.ic.pub.bill.GeneralSqlString.formInSQL("b.csourcebillbid", params));
  ...
  /*************************************************************/
  // 保留的系统管理接口：
  afterCallMethod("nc.bs.ic.pub.bill.GeneralBillDMO", "isSourcesBillSigned", new Object[] { params, sSourceBillType });
  /*************************************************************/
...
//                zhy如果此处不设置setCinbillbodyid的值,则upate bb2表时,就得不到[0]行以外的其他表体行对应的序列号,导致无法保存
                for(int i=0;i<voaSn.length;i++){
                  if(voaSn[i].getCserialid().equals(voTempSN.getCserialid())){
                    voaSn[i].setCgeneralbid(voTempSN.getCgeneralbid());
                    voaSn[i].setCinbillbodyid(voTempSN.getCgeneralbid());
                    break;
                  }
                }
 ...
 * @param sInvID java.lang.String
 */
public String outSerialsWithTest(
  GeneralBillHeaderVO voHeader,
  GeneralBillItemVO voItem,
  Vector vEx,
  Vector vNotEx,
  ...
  SerialDMO dmoSN = new SerialDMO(); //序列号dmo
  //StringBuffer sbSql = new StringBuffer("UPDATE ic_general_bb2 SET ccustomerid=");
  //if (param.get(0) == null)
  //sbSql.append("NULL ");
  //else {
  //sbSql.append("'");
...
//    按库管员过滤单据
      if(curbilltype!=null && curbilltype.startsWith("4") && !curbilltype.equals(BillTypeConst.IC_WAST)){
        voaBody =  ICCommonBusiImpl.filterVoByWhManagerid(curcorp,cwarehouseid,cuserid,voaBody);
      }
...
//  从voConds中得到关联语句：
  ...
  sbSQL.append(" distinct /*+ index(head) */ ");
  try{
...
public nc.vo.pub.CircularlyAccessibleValueObject[] queryAllHeadData(
  java.lang.String whereString,
  String sRetBillTypeCode,
  Integer bRetBillInOutFlag)
  throws BusinessException {
  nc.vo.scm.pub.SCMEnv.out("wo ??" + whereString);
  //自身单据的数量字段名。
...
//      sbSQL.append("  body.nretastnum\n");
  ...
  sbSQL.append(
  " AND (COALESCE(" + sMyNumKey + ",0.0)-COALESCE(ntranoutnum,0.0)-COALESCE(nretnum,0.0))>0 ");
...
public String queryBillCode(String sBillPK) throws Exception {
  ...
  String sRet = BillStatus.FREE;
  String sSql =
    " SELECT vbillcode FROM ic_general_h WHERE dr=0 AND cgeneralhid=?";
 ...
 *  修改日期，修改人，修改原因，注释标志：
 */
public String queryBillHeadTs(String sHeadPK)
  ...
  try {
    ...
    stmt =
      con.prepareStatement(
        "select ts  from ic_general_h where dr=0 AND cgeneralhid = ?");
...
public GeneralBillItemVO queryBillItem(String sBodyPK)
  ...
  StringBuffer sbSql = new StringBuffer("SELECT /*+ index(body) */ ");
  try{
      sbSql.append(GenMethod.getSelectFields(GeneralBillItemVO.class,null));
  }catch(Exception e){
      throw new BusinessException(e.getMessage());
  }
  sbSql.append(",(coalesce(ninnum,noutnum,0.0) - coalesce(nretnum,0.0) - coalesce(ntranoutnum,0.0)) as nleftnum");
  sbSql.append(",(coalesce(ninassistnum,noutassistnum,0.0) - coalesce(nretastnum,0.0) - coalesce(ntranoutastnum,0.0)) as nleftastnum");
  sbSql.append(",(coalesce(ningrossnum,noutgrossnum,0.0) - coalesce(nretgrossnum,0.0)) as nleftgrsnum");
 ...
 * 此处插入方法说明。
   查询指定单据的条码
   通过单据单据ArrayList
 * 创建日期：(2004-3-24 13:48:01)
 * @param ArrayList alBillVO 单据ArrayList *
 */
  ...
  try {
    if (alBillVO != null && alBillVO.size() > 0) {
      ...
      if (alBillItem != null && alBillItem.size() > 0) {
        GeneralBillItemVO[] billAllItemvos = new GeneralBillItemVO[alBillItem.size()];
 ...
 * 此处插入方法说明。
   ...
   通过单据表体ID
 * 创建日期：(2004-3-24 13:48:01)
 * @param voaItem nc.vo.ic.pub.bill.GeneralBillItemVO[]
 * @param saBillPK java.lang.String[]
 */
public void queryBillItemBarCode(nc.vo.ic.pub.IBillItemBarcodeVO[] voaItem) throws Exception {
  ...
  BarCodeVO[] barcodeVOs = barCodeDMO.queryAllByBIDs(sBillItemID);
  ...
  for (int i = 0; i < voaItem.length; i++) {
    ...
    if (sBID != null && htbBIDBarcodeItems.containsKey(sBID)) {
      ...
      if (alBarcodeVO != null && alBarcodeVO.size() > 0) {
        BarCodeVO[] barcodebillitemVOs = new BarCodeVO[alBarcodeVO.size()];
 ...
 * 创建日期：(2004-3-24 13:48:01)
 * @param voaItem nc.vo.ic.pub.bill.GeneralBillItemVO[]
 * @param saBillPK java.lang.String[] 单据表头ID数组
 */
public void queryBillItemBarCode(nc.vo.ic.pub.IBillItemBarcodeVO[] voaItem, String[] saBillPK) throws Exception {

  if (voaItem == null || voaItem.length == 0 || saBillPK == null || saBillPK.length == 0)
    return;
...
* 参数：
    sHeadPK：表体ID
    boolean bLoadSN:是否读取序列号
    boolean bLoadLocator:是否读取货位
* 返回：
* 例外：
* 日期：(2001-6-12 20:38:02)
* 修改日期，修改人，修改原因，注释标志：
...
* @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
*/
public GeneralBillItemVO[] queryBillItemByBillPk(
  String sHeadPK,
  boolean bLoadSN,
  boolean bLoadLocator)
  ...
  if (voaRetBillItem != null
    && voaRetBillItem.length > 0
    ...
    if (htBB3 != null && htBB3.size() > 0) {
      ...
      for (int i = 0; i < voItems.length; i++) {
        ...
        if (htBB3.containsKey(bid)) {
          vbb3 = (nc.vo.ic.pub.settlement.SettlementVO) htBB3.get(bid);
          voItems[i].setBB3VO(new nc.vo.ic.pub.settlement.SettlementVO[]{vbb3});
...
public GeneralBillItemVO queryBillItemQtyInfo(String sBodyPK)
  ...
  try {
    ...
    stmt =
      con.prepareStatement(
        "SELECT  noutnum, noutassistnum,  ninnum, ninassistnum FROM ic_general_b WHERE dr=0 AND cgeneralbid = ?");
    stmt.setString(1, sBodyPK);
    rs = stmt.executeQuery();
    nc.bs.ic.pub.GenMethod gm = new nc.bs.ic.pub.GenMethod();
    ...
    if (rs.next())
      gm.setData(
        ...
        1,
        4,
        new String[] { "noutnum", "noutassistnum", "ninnum", "ninassistnum" });
...
/**
 * 此处插入方法说明。
  通过单据的表头ID查询表体ID,来源单据ID
 * 创建日期：(2004-4-20   16:28:53)
 * @return java.lang.String
 * @param sBID java.lang.String 单据的表体ID,
 * @param sSourceID java.lang.String来源单据ID
 */
public HashMap queryBillItemSourceIDs(String sHIDsubSql,String sSourdeBillType) throws Exception {
  ...
  String sql =
    "select cgeneralbid,"+sCsourceIDfield+" from ic_general_b where cgeneralhid in "
      + sHIDsubSql
...
public GeneralBillItemVO[] queryBillItemTs(String sHeadPK)
  ...
  try {
    ...
    stmt =
      con.prepareStatement(
        "select cgeneralbid,ts from ic_general_b where dr=0 AND cgeneralhid = ?");
    stmt.setString(1, sHeadPK);
    rs = stmt.executeQuery();
    while (rs.next()) {
 ...
 * 日期：(2001-6-12 20:38:02)
 * 修改日期，修改人，修改原因，注释标志：
  2002-01-16 , 王乃军，移植到oracle


 * @return nc.vo.ic.pub.bill.GeneralBillVO[]
 * @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
 */
public ArrayList queryBillByPks(String[] pks)
  throws BusinessException {
    ...
    switch (iQryMode) {
      ...
      case GeneralBillVO.QRY_ITEM_ONLY_PURE :
        if (voQryCond.getParam(0) == null)
          ...
          GeneralBillItemVO[] voaAllItem =
            (GeneralBillItemVO[]) queryPureItemData(saBillPK);
          //实现查询条码数据
...
/**
 * 生产制造接口使用方法.
 * 功能：通过表体的来源单据Bid, 得到多张单据的Hid, 操作员ID, 公司ID
 * 参数：
 * 返回：
 * 例外：
 * 日期：(2001-10-18 12:44:36)
...
public ArrayList queryBillsBySourceBIDs(String[] sourceHeadIDs)
  ...
  // 保留的系统管理接口：
  beforeCallMethod(
    "nc.bs.ic.ic211.GeneralHDMO",
    "queryBillsBySourceBIDs",
  ...
  StringBuffer sql =
    new StringBuffer("select distinct h.cgeneralhid, coperatorid,h.pk_corp from ic_general_h h inner join ic_general_b b on(h.cgeneralhid = b.cgeneralhid) where b.dr=0 AND h.dr=0 AND b.csourcebillbid in(");
...
/**
 * 生产制造接口使用方法.
 * 功能：通过表体的来源单据Hid, 得到多张单据的Hid, 操作员ID, 公司ID
 * 参数：
 * 返回：
 * 例外：
 * 日期：(2001-10-18 12:44:36)
 * 修改日期，修改人，修改原因，注释标志：
 */
public ArrayList queryBillsBySourceHID(String sourceHeadID) throws SQLException {
  ...
  String sql =
    "select distinct h.cgeneralhid, coperatorid,h.pk_corp from ic_general_h h inner join ic_general_b b on(h.cgeneralhid = b.cgeneralhid) where b.dr=0 AND h.dr=0 AND b.csourcebillhid = ? ";
  ...
  // 保留的系统管理接口：
  afterCallMethod(
    "nc.bs.ic.pub.settlement.SettlementDMO",
    "findByPrimaryKey",
    new Object[] { sourceHeadID });
  /*************************************************************/
...
/**
 * 销售接口使用方法.
 * 功能：通过表体的来源单据Hid, 得到单据的Hid, 操作员ID, 公司ID
 * 参数：
 * 返回：
 * 例外：
 * 日期：(2001-10-18 12:44:36)
 ...
 * @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
 */
protected ArrayList queryBySql(
  String sSql,
  int iHeadFieldNum,
  int iBodyFieldNum)
  throws Exception {
      ...
      if (sCurHeadPK != null && !sCurHeadPK.equals(sHeadPK)) {
        ...
        if (voBill != null) {
          ...
          if (vResultItemData.size() < 1) {
            nc.vo.scm.pub.SCMEnv.out(voBillHead.getPrimaryKey() + "bill has no item ERROR!");
            return null;
          }
...
public ArrayList queryChildrenBillInfo(String sParentBillPK)
  ...
  try {
    ...
    stmt =
      con.prepareStatement(
        "SELECT DISTINCT h.cgeneralhid,bt.billtypename,h.vbillcode FROM ic_general_h h INNER JOIN ic_general_b b ON h.cgeneralhid=b.cgeneralhid \n LEFT OUTER JOIN bd_billtype bt ON h.cbilltypecode=bt.pk_billtypecode WHERE b.dr=0 AND h.dr=0 AND b.csourcebillhid=?");
...
* @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
*/
public String[] queryChildrenBillPK(String sParentBillPK)
  throws Exception {
  Connection con = null;
  PreparedStatement stmt = null;
  ResultSet rs = null;
  ...
  try {
    ...
    stmt =
      con.prepareStatement(
        "SELECT DISTINCT h.cgeneralhid FROM ic_general_h h INNER JOIN ic_general_b b ON h.cgeneralhid=b.cgeneralhid WHERE h.dr=0 AND b.dr=0 AND  b.csourcebillhid=?");
    stmt.setString(1, sParentBillPK);
    rs = stmt.executeQuery();
    Vector vRet = new Vector();
    String sValue = null;
 ...
 *  修改日期，修改人，修改原因，注释标志：
 */
public Hashtable queryHashHeadTs(ArrayList alhid)
  throws Exception {
...
public GeneralBillHeaderVO[] queryHeadDataBySql(String sSql)
  ...
  try {
    ...
    nc.vo.scm.pub.SCMEnv.out("---------------set value use time is " + (end - begin));
  }catch (SQLException e){
 ...
 * @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
 */
public GeneralBillVO[] queryInitBills(String sCorpID)
  throws SQLException, BusinessException, Exception {
  if (sCorpID == null || sCorpID.trim().length() == 0)
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000209")/*@res "查询期初单据时参数公司为空。"*/);
  //查期初单据
  ArrayList alRet =
    queryBills(
      new QryConditionVO(
        " head.pk_corp='"
          ...
          + "') and head.cwarehouseid in (select pk_stordoc from bd_stordoc where pk_corp='"+sCorpID+"' and iscalculatedinvcost='Y') \n "));
  //返回数组
...
/**
 * 创建者：王乃军
 * 功能：读指定单据PK的单据行数据，无还回等数据
 * 参数：
  String saBillPK[]，单据pk
 ...
 * 修改日期，修改人，修改原因，注释标志：
 */
public GeneralBillItemVO[] queryItemData(String saBillPK[]) throws SQLException {
...
//  //------- where -------------
//  sbSql.append("\n    WHERE  body.dr=0  AND " + GeneralSqlString.leftOuterJoinConvert("conv", "conv102"));
//
//  //使用临时表
 ...
 * 返回：
 * 例外：
 * 日期：(2002-5-13 11:40:52)
 * 修改日期，修改人，修改原因，注释标志：
 * @return java.util.ArrayList
 * @param cgeneralbid java.lang.String
 * @exception nc.vo.pub.BusinessException 异常说明。
 */
public Hashtable queryItemForATP(String cgeneralhid) throws nc.vo.pub.BusinessException {
    Hashtable ht=new Hashtable();
  if(cgeneralhid==null)
    return ht;
  ...
  try {
    ...
    while (rs.next()) {
      ...
      boolean isNull=true;
      for(int i=0;i<10;i++){
        if(freeitems[i]!=null&&freeitems[i].trim().length()>0){
      ...
      alRet.add(uffalse);
          if(flargess!=null&&flargess.equalsIgnoreCase("Y")){
            if(csourcetype!=null&&(csourcetype.equals("21")||csourcetype.equals("23"))){
...
/**
 * 此处插入方法说明。
 * 创建日期：(2001-7-6 11:05:27)
 * 查询非货位仓库辅计量管理的存货是否数量同时为0
 */
private Hashtable queryNotAllZero(GeneralBillVO voBill) throws java.sql.SQLException {
  ...
  for(int i=0;i<voItems.length;i++){
    if(voItems[i].getCastunitid()!=null
        ...
        &&((UFBoolean)voItems[i].getAttributeValue("autobalancemeas")).booleanValue())
  ...
  Hashtable ht=new Hashtable();
  StringBuffer sql=new StringBuffer("select cinventoryid,vfree1,vfree2,vfree3,vfree4,vfree5,vfree6,vfree7,vfree8,vfree9,vfree10,vbatchcode,castunitid ");
  sql.append(",cvendorid,hsl");
  sql.append(",isnull(num,0) as ninnum,isnull(assistnum,0) as ninassistnum from ");
  sql.append(" (select cinventoryid,vfree1,vfree2,vfree3,vfree4,vfree5,vfree6,vfree7,vfree8,vfree9,vfree10,vlot as vbatchcode,castunitid,");
  sql.append(" cvendorid,hsl,");
  sql.append(" sum(isnull(nonhandnum,0)) as num, sum(isnull(nonhandastnum,0)) as assistnum ");
  sql.append(" from "+GeneralSqlString.V_IC_ONHANDNUM+" onhand ");
  sql.append(" where cwarehouseid=? ");
  sql.append(GeneralSqlString.formInSQL("cinventoryid",alData));//and cinventoryid in ")
//  .append(" (select cinventoryid from ic_general_b where cgeneralhid=? and castunitid is not null and dr=0)")
  sql.append(" group by cinventoryid,vfree1,vfree2,vfree3,vfree4,vfree5,vfree6,vfree7,vfree8,vfree9,vfree10,vlot,castunitid");
  sql.append(",cvendorid,hsl");
  sql.append(" ) a ");
  sql.append(" where (num=0 and assistnum<>0) or (num<>0 and assistnum=0)");
  ...
  try {
    ...
    while (rs.next()) {
      ...
      gm.setData(rs,voItem,meta);
      key=new StringBuffer();
      key.append(voItem.getCinventoryid())
      .append(voItem.getVfree1())
      .append(voItem.getVfree2())
      .append(voItem.getVfree3())
      .append(voItem.getVfree4())
      .append(voItem.getVfree5())
      .append(voItem.getVfree6())
      .append(voItem.getVfree7())
      .append(voItem.getVfree8())
      .append(voItem.getVfree9())
      .append(voItem.getVfree10())
      .append(voItem.getVbatchcode())
      .append(voItem.getCastunitid())
      .append(voItem.getCvendorid());
 ...
 * 此处插入方法说明。
 * 创建日期：(2001-7-6 11:05:27)
 * 查询货位仓库辅计量管理的存货是否数量同时为0
 */
private Hashtable queryNotAllZeroCargo(GeneralBillVO voBill) throws Exception {
  ...
  for(int i=0;i<voItems.length;i++){
    if(voItems[i].getCastunitid()!=null
      ...
      if(voItems[i].getLocator()!=null){
        ...
        for(int j=0;j<voLocs.length;j++){
          key1=voItems[i].getCinventoryid()+voLocs[j].getCspaceid();
  ...
  tmpTable =
    dmo.getTempStringTable(
      getConnection(),
      "ic_tmp_invspace_castunit",
      new String[] { "cinvmanid", "cspaceid" },
      new String[] { "char(20) not null ", "char(20) " },
      null,
      alData);
  if(tmpTable ==null)
  ...
  Hashtable ht=new Hashtable();
  StringBuffer sql=new StringBuffer("select cinventoryid,vfree1,vfree2,vfree3,vfree4,vfree5,vfree6,vfree7,vfree8,vfree9,vfree10,vbatchcode,castunitid ,cspaceid")
  .append(",cvendorid,hsl")
  .append(",isnull(num,0) as ninnum,isnull(assistnum,0) as ninassistnum from ")
  .append(" (select cinventoryidb as cinventoryid,vfreeb1 as vfree1,vfreeb2 as vfree2,vfreeb3 as vfree3,vfreeb4 as vfree4,vfreeb5 as vfree5,vfreeb6 as vfree6,vfreeb7 as vfree7,vfreeb8 as vfree8,vfreeb9 as vfree9,vfreeb10 as vfree10,vlotb as vbatchcode,castunitidb as castunitid,onhandnumb.cspaceid as cspaceid ")
  .append(" ,onhandnumb.cvendorid,onhandnumb.hsl,")
  .append(" sum(isnull(nnum,0)) as num, sum(isnull(nastnum,0)) as assistnum ")
  .append(" from "+GeneralSqlString.V_IC_ONHANDNUM_B+" onhandnumb ,"+tmpTable+" tmpb ")
  .append(" where onhandnumb.cinventoryidb=tmpb.cinvmanid and onhandnumb.cspaceid=tmpb.cspaceid  ")
//  .append(" and cspaceid in (select cspaceid from ic_general_bb1 where cgeneralbid in (select cgeneralbid from ic_general_b where cgeneralhid=? and dr=0 ) and dr=0 ) ")
//  .append(" and cinventoryidb in (select cinventoryid from ic_general_b where cgeneralhid=? and castunitid is not null)")
  .append(" group by cinventoryidb,vfreeb1,vfreeb2,vfreeb3,vfreeb4,vfreeb5,vfreeb6,vfreeb7,vfreeb8,vfreeb9,vfreeb10,vlotb,castunitidb,onhandnumb.cspaceid ")
  .append(" ,onhandnumb.cvendorid,onhandnumb.hsl ")
  .append(" ) a ")
...
/**
 * 方法说明：查询纯的单据字段，用动态关联表查询。
 * 创建日期：(2003-1-15 9:57:19)
 * 作者：程起伍
 * 修改日期：
 * 修改人：
 * 修改原因：
 ...
 * @param param nc.vo.pub.query.ConditionVO
 * @exception java.sql.SQLException 异常说明。
 */
protected ArrayList queryPureHeadersAndPartItems(QryConditionVO voQryCond) throws Exception{
 ...
 * @return
 */
 ArrayList queryPureHeadersPartItems(QryConditionVO voQryCond,boolean isWhole) throws Exception{
    ArrayList alRet = null; //返回值
  try {
    ...
    isJoin = dynamicSQL.isJoinBody(voConds);
    ...
    if (isSearchBCOpen == true){
      sbSQL.append(" AND body.cinventoryid in (select pk_invmandoc from bd_invmandoc where isprimarybarcode='Y' or issecondarybarcode='Y')");
    }
    //动态关联结束
    ...
    Timer t = new Timer();
    t.start();
    GeneralBillHeaderVO[] voaHeader = (GeneralBillHeaderVO[]) queryHeadDataBySql(sbSQL.toString());
    ...
    if (voaHeader != null && voaHeader.length > 0) {
      //需要读表体的数量
        ...
        int iNeedItemNum = 0;
        if(!isWhole)
            iNeedItemNum = Math.min(GeneralBillVO.QRY_FIRST_ITEM_NUM, voaHeader.length);
        else
 ...
 * 返回：
 * 例外：
 * 日期：(2003-2-14 9:07:50)
 * 修改日期，修改人，修改原因，注释标志：
 */
public GeneralBillItemVO[] queryPureItemData(String saBillPK[]) throws BusinessException {
  if (saBillPK == null || saBillPK.length == 0 || saBillPK[0] == null || saBillPK[0].trim().length() == 0)
    return null;
  try {
    StringBuffer sbSql = new StringBuffer("SELECT /*+ index(body) */ ");
    ...
    //使用临时表
    sbSql.append(" AND body.dr=0 ORDER BY body.cgeneralhid,cast(body.crowno as numeric(20,8)) \n");
...
/**
 * 创建者：程起伍
 * 功能：读指定单据PK的单据行数据[主要用于平台上下游单据查询。]
 * 参数：
  String saBillPK[]，单据pk
  String sBodyWhere
 ...
 * 修改日期，修改人，修改原因，注释标志：
 */
public GeneralBillItemVO[] queryPureItemData(String saBillPK[],String sBodyWhere) throws Exception {
  if (saBillPK == null || saBillPK.length == 0 || saBillPK[0] == null || saBillPK[0].trim().length() == 0)
    return null;
  //  从voConds中得到关联语句：
  DynamicJoinSql dynamicSQL = new DynamicJoinSql("head", "body");
    ...
    if(albill != null && albill.size()>0){
      ...
      for(int i=0;i<ilen;i++){
        ...
        for(int j=0;j<bsize;j++)
          alBid.add(((GeneralBillVO)albill.get(i)).getItemVOs()[j].getCgeneralbid());
        alHid.add(((GeneralBillVO)albill.get(i)).getHeaderVO().getCgeneralhid());
      }
      ...
      for(int i=0;i<ilen;i++){
        ...
        for(int j=0;j<bsize;j++){
          ...
          if(htLocator.containsKey(cgeneralbid)){
            ...
            if(alLocator != null){
              ...
              if(voaLocator.length ==1){
                voaItem[j].setCspaceid(voaLocator[0].getCspaceid());
                voaItem[j].setVspacename(voaLocator[0].getVspacename());
              }
            }
          }
          if(htSn.containsKey(cgeneralbid)){
...
//  sbSql.append(GenMethod.getSelectFields(GeneralBillItemVO.class,"body"));
//  sbSql.append(
//    " FROM ic_general_h head INNER  JOIN ic_general_b body ON head.cgeneralhid=body.cgeneralhid ");
//  //从voQryCond中得到ConditionVO
//  ConditionVO[] voConds =
...
//  sbSql.append(" ORDER BY head.cgeneralhid, cast(body.crowno as numeric(20,8)) \n");
//  return queryBySql(
//    sbSql.toString(),
...
* @param voQryCond nc.vo.ic.pub.bill.QryConditionVO
*/
public ArrayList queryRelatedBillInfoByBodyPK(String sBillPK, String sBillBodyPK) throws Exception {
  Connection con = null;
  PreparedStatement stmt = null;
  ResultSet rs = null;
  //return value
 ...
 * @param sSourceID java.lang.String来源单据ID
 */
public String querySourceHID(String sBID, String sSourceID) throws Exception {
...
/**
 * 此处插入方法说明。
  通过单据的表体ID,来源单据查找转库单、调拨单对应的多个的表头ID
 * 创建日期：(2004-4-20   16:28:53)
 * @return ArrayList
 * @param sBID java.lang.String 单据的表体ID,
 * @param sSourceID java.lang.String来源单据ID
 bHasBID:当前表体是否有ID,false表示新增情况的的单据
 sCsourceType:来源单据类型
 如果为：
  4K 转库单从来源单据字段查询
 否则
 ...
 */
public ArrayList querySourceHIDs(
  String sSourceID,
  ...
  sql =
    "select distinct h.cgeneralhid from ic_general_b b,ic_general_h h where h.cgeneralhid=b.cgeneralhid and b."+sCsourceIDfield+" =?  and h.cbilltypecode<>'"
      + sBilltypecode
...
public ArrayList querySourceHIDs(
  ...
  sql =
    "select distinct h.cgeneralhid from ic_general_b b,ic_general_h h where h.cgeneralhid=b.cgeneralhid and h.cbilltypecode<>'"
      + sBilltypecode
      + "'   and b.dr=0  and h.dr=0"
 ...
 * 功能描述:
 noutnum+onhandnum<0
 ninnum-num>0 时，本行数量直接销减

 noutnum+onhandnum>0
 ninnum-num<0 时，本行数量不够，需要多次销减
 ...
 * 作者:王乃军
 * 日期:
 * @param voItemCur nc.vo.ic.pub.bill.GeneralBillItemVO
 * @param voItemChanged nc.vo.ic.pub.bill.GeneralBillItemVO
 */
private void relation(
  GeneralBillItemVO voNewItem,
  ...
  if (voNewItem.getNinnum() != null) {
    ...
    if (num != null && num.compareTo(ZERO) != 0.0) {
      ...
      if(isEnough){
        ...
        voChangedItem.setNinassistnum(assistnum.sub(voNewItem.getNinassistnum()));
  ...
  //出库单据
    ...
    if (num != null && num.compareTo(ZERO) != 0.0) {
      ...
      if(isEnough){
        ...
        voChangedItem.setNinassistnum(assistnum.add(voNewItem.getNoutassistnum()));
 ...
 * 功能描述:
  //如果入库单，并且入库业务6，
    结存值>=o 5,
      6,5 本行够，
      如果ninnum-num>=0,ninnum-num(6-5),ninspacenum(6)-num(5),voChangedItem .ninnum=0;
      6,8 本行不够
  ...
  //如果入库单，并且出库业务-6，
    结存值<o,
      -6,-5 本行够，
      如果ninnum-num<=0,ninnum(-6)-num(-5),noutspacenum(6)+num(-5),voChangedItem .ninnum=0;
      -6,-8 本行不够
...
private void relation2(
  ...
  if((num==null||num.doubleValue()==0.0)&&(assistnum==null||assistnum.doubleValue()==0.0))
    return;
  ...
  if(voNewItem.getNinnum()!=null&&voLoc.getNinspacenum()!=null){
    ...
    if(assistnum!=null&&assistnum.compareTo(ZERO)>=0){
      ...
      //如果ninassistnum-assistnum>=0,ninassistnum-assistnum(6-5),ninspaceassistnum(6)-assistnum(5),voChangedItem .ninassistnum=0;
      if(voNewItem.getNinassistnum().sub(assistnum).compareTo(ZERO)>=0){
        voNewItem.setNinassistnum(voNewItem.getNinassistnum().sub(assistnum));
        voLoc.setNinspaceassistnum(voLoc.getNinspaceassistnum().sub(assistnum));
  ...
  if(voNewItem.getNinnum()!=null&&voLoc.getNoutspacenum()!=null){
    ...
    if(assistnum!=null&&assistnum.compareTo(ZERO)<=0){
      ...
      //如果ninassistnum-assistnum<=0,ninassistnum(-6)-assistnum(-5),noutspaceassistnum(6)+assistnum(-5),voChangedItem .ninassistnum=0;
      if(voNewItem.getNinassistnum().sub(assistnum).compareTo(ZERO)<=0){
        voNewItem.setNinassistnum(voNewItem.getNinassistnum().sub(assistnum));
        voLoc.setNoutspaceassistnum(voLoc.getNoutspaceassistnum().add(assistnum));
        voChangedItem.setNinassistnum(ZERO);
      }
  ...
  if(voNewItem.getNoutnum()!=null&&voLoc.getNinspacenum()!=null){
    ...
    if(assistnum!=null&&assistnum.compareTo(ZERO)>=0){
      ...
      if(voNewItem.getNoutassistnum().add(assistnum).compareTo(ZERO)<=0){
        voNewItem.setNoutassistnum(voNewItem.getNoutassistnum().add(assistnum));
        voLoc.setNinspaceassistnum(voLoc.getNinspaceassistnum().sub(assistnum));
        voChangedItem.setNinassistnum(ZERO);
      }
...
/**
 * 李俊
 * 功能：更改表体字段bbarcodeclose
 * 参数：
 * 返回：
 * 例外：
 * 日期：(2004-10-11 13:31:13)
 * 修改日期，修改人，修改原因，注释标志：
 *
 * @param voaItem nc.vo.ic.pub.bill.GeneralBillItemVO[]
 * @param ufBoolean nc.vo.pub.lang.UFBoolean
 */
public void revBBarcodeClose(ArrayList alParam, UFBoolean bBarcodeClose) throws SQLException{
  ...
  /*************************************************************/
  String sql =
    "update ic_general_b set bbarcodeclose=?  where cgeneralbid = ?";

  ArrayList alBids = (ArrayList)alParam.get(1);
  if (alBids==null||alBids.size()<=0) return;
  Connection con = null;
  PreparedStatement stmt = null;
  try {
    con = getConnection();
    ...
    for (int i = 0; i < alBids.size(); i++) {
        ...
        if (sBodyPK==null||sBodyPK.trim().length()==0) continue;
...
/**
 * 此处插入方法说明。
 * 功能描述:为新增条码的纪录预先获取pk，用于独立的保存条码数据
 * 输入参数:
 * 返回值:
 * 异常处理:
 * 日期:
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public ArrayList setBillBarcodePrimaryKeys(GeneralBillItemVO[] voItems,String sPk_corp) throws Exception {
  ...
  for (int i = 0; i < voItems.length; i++) {
    if (voItems[i].getBarCodeVOs() != null) {
      for (int b = 0; b < voItems[i].getBarCodeVOs().length; b++) {
        if (voItems[i].getBarCodeVOs()[b] != null && voItems[i].getBarCodeVOs()[b].getStatus() == VOStatus.NEW) {
          iBodyCount++;
        }
      }
    }
  ...
  if (keys != null && keys.length == iBodyCount) {
      ...
      if (voItems[i].getBarCodeVOs() != null) {
        for (int b = 0; b < voItems[i].getBarCodeVOs().length; b++) {
          if (voItems[i].getBarCodeVOs()[b] != null && voItems[i].getBarCodeVOs()[b].getStatus() == VOStatus.NEW) {
            ...
            voItems[i].getBarCodeVOs()[b].setCgeneralbbcid(keys[iBodyCount - ileftcount]);
            //将对应的表体行主键置入BarCodeVO
            voItems[i].getBarCodeVOs()[b].setCgeneralbid(voItems[i].getPrimaryKey());
    ...
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000210")/*@res "库存：批量获取OID失败！"*/);
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public ArrayList setBillBarcodePrimaryKeys(GeneralBillVO voBill) throws Exception {
  ArrayList alPK = new ArrayList();
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public ArrayList setBillPrimaryKeys(GeneralBillVO voBill) throws Exception {
  ArrayList alPK = new ArrayList();
  if (voBill == null
    || voBill.getParentVO() == null
  ...
  for (int i = 0; i < voItems.length; i++) {
    ...
    /**
     * 修改人：刘家清
	修改日期：2007-5-24下午08:31:00
	修改原因：因为在这个循环当中，bSaveBarcodeFinal()里调用了lookup来判断权限，所以导致了200条数据运行时间由原来的0分0秒16毫秒上升至0分6秒218毫秒
			以后修改中切记不要把操作时间长的东西放到公共类里比如VO里
			
			if (voBill.bSaveBarcodeFinal() && voItems[i].getBarCodeVOs() != null) {
     */
    if ( voItems[i].getBarCodeVOs() != null) {
      for (int b = 0; b < voItems[i].getBarCodeVOs().length; b++) {
        if (voItems[i].getBarCodeVOs()[b]!=null&&(voItems[i].getStatus() == VOStatus.NEW  || voItems[i].getBarCodeVOs()[b].getStatus() == VOStatus.NEW)) {
          iBodyCount++;
        }
  ...
  if (keys != null && keys.length == iBodyCount) {
    ...
    for (int i = 0; i < voItems.length; i++) {
      ...
      if (voItems[i].getStatus() == VOStatus.NEW || bImportdata) {
        ...
        ileftcount--;
        if (hasBB3) {
          voItems[i].setCgeneralbb3(keys[iBodyCount - ileftcount]);
          ileftcount--;
        }
      ...
      if (voItems[i].getLocator() != null) {
        LocatorVO[] voLocs = voItems[i].getLocator();
        for (int j = 0; j < voLocs.length; j++) {
          voLocs[j].setAttributeValue("cgeneralbb1", keys[iBodyCount - ileftcount]);
      ...
      if ( voItems[i].getBarCodeVOs() != null) {
        for (int b = 0; b < voItems[i].getBarCodeVOs().length; b++) {
          if (voItems[i].getBarCodeVOs()[b] != null
            && (voItems[i].getStatus() == VOStatus.NEW || voItems[i].getBarCodeVOs()[b].getStatus() == VOStatus.NEW)) {
            //将批量得到的条码记录的主键置入BarCodeVO
            voItems[i].getBarCodeVOs()[b].setCgeneralbbcid(keys[iBodyCount - ileftcount]);
            //将对应的表体行主键置入BarCodeVO
            voItems[i].getBarCodeVOs()[b].setCgeneralbid(voItems[i].getPrimaryKey());
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public void setBillPrimaryKeysForCopy(GeneralBillVO voBill) throws Exception {
  if (voBill == null
    || voBill.getParentVO() == null
    || voBill.getChildrenVO() == null)
    return;
  ...
  for (int i = 0; i < voItems.length; i++) {
    ...
    if (htBB3 != null && htBB3.size() > 0) {
      ...
      if (htBB3.containsKey(voItems[i].getCgeneralbid())) {
        vTmp=(nc.vo.ic.pub.settlement.SettlementVO)htBB3.get(voItems[i].getCgeneralbid());
  ...
  if (keys != null && keys.length == iBodyCount) {
    ...
    for (int i = 0; i < voItems.length; i++) {
      ...
      if (voItems[i].getBB3VO() != null) {
        nc.vo.ic.pub.settlement.SettlementVO[] voSets = voItems[i].getBB3VO();
 ...
 */
public void setFirstBillCode(GeneralBillItemVO[] voItems,String cgeneralhid) throws Exception{
  if(voItems==null||voItems.length==0||cgeneralhid==null)
    return;

  String billtype=voItems[0].getCfirsttype();

  if(voItems[0].getCfirstbillhid()==null||billtype==null||voItems[0].getVfirstbillcode()!=null)
    return;
  ...
  if(al.size()>0){
    ...
    String subQry="select cfirstbillhid from ic_general_b where cgeneralhid='"+cgeneralhid+"' and dr=0 ";
 ...
 * @param voItems
  根据voItem中的值 cfirstbillhid,和cfirsttype找到cfirstbillcode
 */
public void setFirstBillCode(String billtype,String subQry, String hid) throws Exception {
...
public void setSettledOK(String sBillPK) throws java.sql.SQLException {
  ...
  String sql1 =
    "update ic_general_b set isok = 'Y' where cgeneralhid = ? and dr=0 ";
 ...
 * @param voItems
  根据voItem中的值 cfirstbillhid,和cfirsttype找到cfirstbillcode
 */
public void setSourceRowNos(String billtype,String hid) throws Exception {
  if(billtype==null)
    return;
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public Boolean sign(GeneralBillVO[] voBills)
  throws BusinessException, Exception {
//   保留的系统管理接口：
  beforeCallMethod(
  "nc.bs.ic.pub.bill.GeneralBillDMO",
  ...
  String sql =
    "UPDATE ic_general_h SET  cregister = ?, daccountdate = ? ,fbillflag=?, taccounttime=? WHERE cgeneralhid = ?";
  ...
  try {
    ...
    for (int i = 0; i < voBills.length; i++) {
      ...
      if (voHead.getAttributeValue("taccounttime") == null)
          sAccountTime = (new UFDateTime(System.currentTimeMillis())).toString();
      else 
          sAccountTime = voHead.getAttributeValue("taccounttime").toString();
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public boolean signCheck(GeneralBillVO voBill)
  throws BusinessException, Exception {
  if (voBill == null || voBill.getHeaderVO() == null) {
 ...
 * @param voBill nc.vo.ic.pub.bill.GeneralBillVO
 */
public boolean signCheck(GeneralBillVO[] voBills)
  throws BusinessException, Exception {
  if (voBills == null || voBills.length == 0 || voBills[0] == null) {
    throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000211")/*@res "签字检查：没有可签字的单据！"*/);
    ...
    if (voBills[i] != null && voBills[i].getHeaderVO() != null) {
      ...
      if (voHead.getDaccountdate() == null
        ...
        if(voHead.getClogdatenow()!=null)
          voHead.setDaccountdate(new UFDate(voHead.getClogdatenow()));
      ...
      if (voHead.getCgeneralhid() == null
        ...
        throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000212")/*@res "签字检查：单据签字信息不完整！"*/);
      else {
        alhid.add(voHead.getCgeneralhid());
        if (billtype == null)
          billtype = voHead.getCbilltypecode();
        if (!billtype.equals(voHead.getCbilltypecode()))
          throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000213")/*@res "签字检查：单据类型不相同，不能批签字！"*/);
      }

    } else
      throw new BusinessException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000214")/*@res "签字检查：单据信息不完整！"*/);
...
//  String coperatorid = null;
  ...
  try {
    ...
    String sql1 =
      "select cgeneralhid,vbillcode,coperatorid,cregister,cauditorid  FROM ic_general_h h  WHERE  h.dr=0  ";
  ...
  if (hthid.size() == voBills.length) {
    ...
    for (int i = 0; i < voBills.length; i++) {
      ...
      if (voItems == null || voItems.length == 0) {
        alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":"+ voBills[i].getHeaderVO().getVbillcode() + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000356")/*@res " 表体信息不完整！"*/);
        continue;
      }
      //检查是否已经签字，制单人,
      if (hthid.containsKey(voBills[i].getHeaderVO().getCgeneralhid())) {
...
//        coperatorid = (String) alrow.get(1);
        ...
        if (cauditorid != null) {
          alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":" + billcode + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000357")/*@res " 已经存货记账！"*/);
        } else if (cregister != null) {
          alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":" + billcode + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000358")/*@res " 已经签字！"*/);
        } 
//        else if (
//          !isCanSameOne && coperatorid.equals(voBills[i].getHeaderVO().getCregister())) {
//          alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":" + billcode + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000359")/*@res " 制单人与签字人不能相同！"*/);
//        }
      } else {
        alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":" + voBills[i].getHeaderVO().getVbillcode() + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000360")/*@res " 可能已经删除！"*/);
      }
      //检查表体数量,签字日期必须晚于业务日期
      for (int j = 0; j < voItems.length; j++) {
        if (voItems[j].getStatus() != VOStatus.DELETED
          ...
          alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":" + voBills[i].getHeaderVO().getVbillcode() + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000361")/*@res " 没有填写实收实发数量！"*/);
          break;
        }
        if (voItems[j].getDbizdate() != null
          && voBills[i].getHeaderVO().getDaccountdate().before(voItems[j].getDbizdate())) {
          alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000248")/*@res "单据"*/+":" + voBills[i].getHeaderVO().getVbillcode() + nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000362")/*@res " 签字日期必须晚于出入库日期！"*/);
          break;
        }
    ...
    alErr.add(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000363")/*@res "部分单据可能已经删除！"*/);
  }

  if (alErr != null && alErr.size() > 0) {
    StringBuffer sMsg = new StringBuffer(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000364")/*@res "签字检查错误：\n"*/);
...
/**
 * 创建者：王乃军
 * 功能：sum items belong to one bill to a serial of bills
 * 参数：
 * 返回：返回类单据及的Arraylist，只有表体数据
 * 例外：
 * 日期：(2001-10-18 11:46:08)
 *  修改日期，修改人，修改原因，注释标志：
 */
...
/**
 * 此处插入方法说明。
 * 创建日期：(2003-10-9 13:16:45)
 */
public void updateBasid(String shid) throws Exception {
  //查询有源头单据得表体行
  String sql =null;
  ...
  try {
    ...
    if(shid==null){
      sql =
    "update ic_general_b set cinvbasid=man.pk_invbasdoc from  ic_general_b b , bd_invmandoc man    where b.cinventoryid=man.pk_invmandoc and (cinvbasid is null or b.cinventoryid=b.cinvbasid)";
...
return;
  ...
  public void updateHeader(GeneralBillHeaderVO voGeneralBillHeader)
    ...
    //// 复制数据
    //try{
      //String cgeneralhid=(String)voGeneralBillHeader.getAttributeValue("cgeneralhid");
...
//    String sql =
//      "update ic_general_h set pk_corp = ?, cbiztype = ?, cbilltypecode = ?, vbillcode = ?, dbilldate = ?, cwarehouseid = ?, cdispatcherid = ?, cdptid = ?, cwhsmanagerid = ?, coperatorid = ?, cregister = ?, dauditdate = ?, cauditorid = ?, daccountdate = ?, cinventoryid = ?, ccustomerid = ?, vdiliveraddress = ?, cdilivertypeid = ?, cwastewarehouseid = ?, cbizid = ?, cproviderid = ?, vnote = ?,  vuserdef1 = ?, vuserdef2 = ?, vuserdef3 = ?, vuserdef4 = ?, vuserdef5 = ?, vuserdef6 = ?, vuserdef7 = ?, vuserdef8 = ?, vuserdef9 = ?, vuserdef10 = ? ,vheadnote2 =? ,ndiscountmny=?,nnetmny=?, cotherwhid=?,cendreportid=?,pk_measware=?,clastmodiid=?,tlastmoditime=?,cothercalbodyid=?,cothercorpid=?,vuserdef11=?,vuserdef12=?,vuserdef13=?,vuserdef14=?,vuserdef15=?,vuserdef16=?,vuserdef17=?,vuserdef18=?,vuserdef19=?,vuserdef20=?,pk_defdoc1=?,pk_defdoc2=?,pk_defdoc3=?,pk_defdoc4=?,pk_defdoc5=?,pk_defdoc6=?,pk_defdoc7=?,pk_defdoc8=?,pk_defdoc9=?,pk_defdoc10=?,pk_defdoc11=?,pk_defdoc12=?,pk_defdoc13=?,pk_defdoc14=?,pk_defdoc15=?,pk_defdoc16=?,pk_defdoc17=?,pk_defdoc18=?,pk_defdoc19=?,pk_defdoc20=?,pk_cubasdoc=?,pk_cubasdocC=? where cgeneralhid = ?";
//   
    //edit by zwx 2016-1-27 更新盖子合计数
    String sql =
        "update ic_general_h set pk_corp = ?, cbiztype = ?, cbilltypecode = ?, vbillcode = ?, dbilldate = ?, cwarehouseid = ?, cdispatcherid = ?, cdptid = ?, cwhsmanagerid = ?, coperatorid = ?, cregister = ?, dauditdate = ?, cauditorid = ?, daccountdate = ?, cinventoryid = ?, ccustomerid = ?, vdiliveraddress = ?, cdilivertypeid = ?, cwastewarehouseid = ?, cbizid = ?, cproviderid = ?, vnote = ?,  vuserdef1 = ?, vuserdef2 = ?, vuserdef3 = ?, vuserdef4 = ?, vuserdef5 = ?, vuserdef6 = ?, vuserdef7 = ?, vuserdef8 = ?, vuserdef9 = ?, vuserdef10 = ? ,vheadnote2 =? ,ndiscountmny=?,nnetmny=?, cotherwhid=?,cendreportid=?,pk_measware=?,clastmodiid=?,tlastmoditime=?,cothercalbodyid=?,cothercorpid=?,vuserdef11=?,vuserdef12=?,vuserdef13=?,vuserdef14=?,vuserdef15=?,vuserdef16=?,vuserdef17=?,vuserdef18=?,vuserdef19=?,vuserdef20=?,pk_defdoc1=?,pk_defdoc2=?,pk_defdoc3=?,pk_defdoc4=?,pk_defdoc5=?,pk_defdoc6=?,pk_defdoc7=?,pk_defdoc8=?,pk_defdoc9=?,pk_defdoc10=?,pk_defdoc11=?,pk_defdoc12=?,pk_defdoc13=?,pk_defdoc14=?,pk_defdoc15=?,pk_defdoc16=?,pk_defdoc17=?,pk_defdoc18=?,pk_defdoc19=?,pk_defdoc20=?,pk_cubasdoc=?,pk_cubasdocC=?  ,gzhjs = ?, gzds = ?,gzfys = ?, gzkbs = ?, gghjs = ?, ggds = ?,ggfys = ?,ggkbs = ? where cgeneralhid = ?";
    ...
    try {
      ...
      if (voGeneralBillHeader.getPk_corp() == null) {
        stmt.setNull(1, Types.CHAR);
      } else {
        stmt.setString(1, voGeneralBillHeader.getPk_corp());
      }
      if (voGeneralBillHeader.getCbiztypeid() == null) {
        stmt.setNull(2, Types.CHAR);
      } else {
        stmt.setString(2, voGeneralBillHeader.getCbiztypeid());
      }
      if (voGeneralBillHeader.getCbilltypecode() == null) {
        stmt.setNull(3, Types.CHAR);
      } else {
        stmt.setString(3, voGeneralBillHeader.getCbilltypecode());
      }
      if (voGeneralBillHeader.getVbillcode() == null) {
        stmt.setNull(4, Types.CHAR);
      } else {
        stmt.setString(4, voGeneralBillHeader.getVbillcode());
      }
      if (voGeneralBillHeader.getDbilldate() == null) {
        stmt.setNull(5, Types.CHAR);
      } else {
        stmt.setString(5, voGeneralBillHeader.getDbilldate().toString());
      }
      if (voGeneralBillHeader.getCwarehouseid() == null) {
        stmt.setNull(6, Types.CHAR);
      } else {
        stmt.setString(6, voGeneralBillHeader.getCwarehouseid());
      }
      if (voGeneralBillHeader.getCdispatcherid() == null) {
        stmt.setNull(7, Types.CHAR);
      } else {
        stmt.setString(7, voGeneralBillHeader.getCdispatcherid());
      }
      if (voGeneralBillHeader.getCdptid() == null) {
        stmt.setNull(8, Types.CHAR);
      } else {
        stmt.setString(8, voGeneralBillHeader.getCdptid());
      }
      if (voGeneralBillHeader.getCwhsmanagerid() == null) {
        stmt.setNull(9, Types.CHAR);
      } else {
        stmt.setString(9, voGeneralBillHeader.getCwhsmanagerid());
      }
      if (voGeneralBillHeader.getCoperatorid() == null) {
        stmt.setNull(10, Types.CHAR);
      } else {
        stmt.setString(10, voGeneralBillHeader.getCoperatorid());
      }
      //if (voGeneralBillHeader.getCregister() == null) {
        stmt.setNull(11, Types.CHAR);
      //} else {
        //stmt.setString(11, voGeneralBillHeader.getCregister());
      //}
      //if (voGeneralBillHeader.getDauditdate() == null) {
        stmt.setNull(12, Types.CHAR);
      //} else {
        //stmt.setString(12, voGeneralBillHeader.getDauditdate().toString());
      //}
      //if (voGeneralBillHeader.getCauditorid() == null) {
        stmt.setNull(13, Types.CHAR);
      //} else {
        //stmt.setString(13, voGeneralBillHeader.getCauditorid());
      //}
      //if (voGeneralBillHeader.getDaccountdate() == null) {
        stmt.setNull(14, Types.CHAR);
      //} else {
        //stmt.setString(14, voGeneralBillHeader.getDaccountdate().toString());
      //}
      if (voGeneralBillHeader.getCinventoryid() == null) {
        stmt.setNull(15, Types.CHAR);
      } else {
        stmt.setString(15, voGeneralBillHeader.getCinventoryid());
      }
      if (voGeneralBillHeader.getCcustomerid() == null) {
        stmt.setNull(16, Types.CHAR);
      } else {
        stmt.setString(16, voGeneralBillHeader.getCcustomerid());
      }
      if (voGeneralBillHeader.getVdiliveraddress() == null) {
        stmt.setNull(17, Types.CHAR);
      } else {
        stmt.setString(17, voGeneralBillHeader.getVdiliveraddress());
      }
      if (voGeneralBillHeader.getCdilivertypeid() == null) {
        stmt.setNull(18, Types.CHAR);
      } else {
        stmt.setString(18, voGeneralBillHeader.getCdilivertypeid());
      }
      if (voGeneralBillHeader.getCwastewarehouseid() == null) {
        stmt.setNull(19, Types.CHAR);
      } else {
        stmt.setString(19, voGeneralBillHeader.getCwastewarehouseid());
      }
      if (voGeneralBillHeader.getCbizid() == null) {
        stmt.setNull(20, Types.CHAR);
      } else {
        stmt.setString(20, voGeneralBillHeader.getCbizid());
      }
      if (voGeneralBillHeader.getCproviderid() == null) {
        stmt.setNull(21, Types.CHAR);
      } else {
        stmt.setString(21, voGeneralBillHeader.getCproviderid());
      }
      if (voGeneralBillHeader.getVnote() == null) {
        stmt.setNull(22, Types.CHAR);
      } else {
        stmt.setString(22, voGeneralBillHeader.getVnote());
      }
      //if (voGeneralBillHeader.getFbillflag() == null) {
      //  stmt.setNull(23, Types.INTEGER);
      //} else {
      //  stmt.setInt(23, voGeneralBillHeader.getFbillflag().intValue());
      //}
      //if (voGeneralBillHeader.getFspecialflag() == null) {
      //  stmt.setNull(24, Types.INTEGER);
      //} else {
      //  stmt.setInt(24, voGeneralBillHeader.getFspecialflag().intValue());
      //}
      if (voGeneralBillHeader.getVuserdef1() == null) {
        stmt.setNull(23, Types.CHAR);
      } else {
        stmt.setString(23, voGeneralBillHeader.getVuserdef1());
      }
      if (voGeneralBillHeader.getVuserdef2() == null) {
        stmt.setNull(24, Types.CHAR);
      } else {
        stmt.setString(24, voGeneralBillHeader.getVuserdef2());
      }
      if (voGeneralBillHeader.getVuserdef3() == null) {
        stmt.setNull(25, Types.CHAR);
      } else {
        stmt.setString(25, voGeneralBillHeader.getVuserdef3());
      }
      if (voGeneralBillHeader.getVuserdef4() == null) {
        stmt.setNull(26, Types.CHAR);
      } else {
        stmt.setString(26, voGeneralBillHeader.getVuserdef4());
      }
      if (voGeneralBillHeader.getVuserdef5() == null) {
        stmt.setNull(27, Types.CHAR);
      } else {
        stmt.setString(27, voGeneralBillHeader.getVuserdef5());
      }
      if (voGeneralBillHeader.getVuserdef6() == null) {
        stmt.setNull(28, Types.CHAR);
      } else {
        stmt.setString(28, voGeneralBillHeader.getVuserdef6());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef7") == null) {
        stmt.setNull(29, Types.CHAR);
      } else {
        stmt.setString(29, voGeneralBillHeader.getAttributeValue("vuserdef7").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef8") == null) {
        stmt.setNull(30, Types.CHAR);
      } else {
        stmt.setString(30, voGeneralBillHeader.getAttributeValue("vuserdef8").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef9") == null) {
        stmt.setNull(31, Types.CHAR);
      } else {
        stmt.setString(31, voGeneralBillHeader.getAttributeValue("vuserdef9").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef10") == null) {
        stmt.setNull(32, Types.CHAR);
      } else {
        stmt.setString(32, voGeneralBillHeader.getAttributeValue("vuserdef10").toString());
      }
      //退货备注 add by hanwei 2003-10-10
      if (voGeneralBillHeader.getVheadnote2() == null) {
        stmt.setNull(33, Types.CHAR);
      } else {
        stmt.setString(33, voGeneralBillHeader.getVheadnote2().trim());
      }
      if (voGeneralBillHeader.getNdiscountmny() == null) {
        stmt.setNull(34, Types.DECIMAL);
      } else {
        stmt.setBigDecimal(34,voGeneralBillHeader.getNdiscountmny().toBigDecimal());
      }
      if (voGeneralBillHeader.getNnetmny() == null) {
        stmt.setNull(35, Types.DECIMAL);
      } else {
        stmt.setBigDecimal(35,voGeneralBillHeader.getNnetmny().toBigDecimal());
      }

      //其他仓库
      if (voGeneralBillHeader.getAttributeValue("cotherwhid") == null){
          stmt.setNull(36, Types.CHAR);
      } else {
          stmt.setString(36, voGeneralBillHeader.getAttributeValue("cotherwhid").toString());
      }
      //完工报告ID
      if (voGeneralBillHeader.getAttributeValue("cendreportid") == null){
          stmt.setNull(37, Types.CHAR);
      } else {
          stmt.setString(37, voGeneralBillHeader.getAttributeValue("cendreportid").toString());
      }
      //计量器具ID
      if (voGeneralBillHeader.getAttributeValue("pk_measware") == null){
          stmt.setNull(38, Types.CHAR);
      } else {
          stmt.setString(38, voGeneralBillHeader.getAttributeValue("pk_measware").toString());
      }
      //最后修改人
      if (voGeneralBillHeader.getAttributeValue("clastmodiid") == null){
          stmt.setNull(39, Types.CHAR);
      } else {
          stmt.setString(39, voGeneralBillHeader.getAttributeValue("clastmodiid").toString());
      }
      //最后修改时间
      if (voGeneralBillHeader.getAttributeValue("tlastmoditime") == null){
          stmt.setNull(40, Types.CHAR);
      } else {
          stmt.setString(40, voGeneralBillHeader.getAttributeValue("tlastmoditime").toString());
      }
      //其他库存组织
      if (voGeneralBillHeader.getAttributeValue("cothercalbodyid") == null){
          stmt.setNull(41, Types.CHAR);
      } else {
          stmt.setString(41, voGeneralBillHeader.getAttributeValue("cothercalbodyid").toString());
      }
      //其他公司
      if (voGeneralBillHeader.getAttributeValue("cothercorpid") == null){
          stmt.setNull(42, Types.CHAR);
      } else {
          stmt.setString(42, voGeneralBillHeader.getAttributeValue("cothercorpid").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef11") == null) {
        stmt.setNull(43, Types.CHAR);
      } else {
        stmt.setString(43, voGeneralBillHeader.getAttributeValue("vuserdef11").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef12") == null) {
        stmt.setNull(44, Types.CHAR);
      } else {
        stmt.setString(44, voGeneralBillHeader.getAttributeValue("vuserdef12").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef13") == null) {
        stmt.setNull(45, Types.CHAR);
      } else {
        stmt.setString(45, voGeneralBillHeader.getAttributeValue("vuserdef13").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef14") == null) {
        stmt.setNull(46, Types.CHAR);
      } else {
        stmt.setString(46, voGeneralBillHeader.getAttributeValue("vuserdef14").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef15") == null) {
        stmt.setNull(47, Types.CHAR);
      } else {
        stmt.setString(47, voGeneralBillHeader.getAttributeValue("vuserdef15").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef16") == null) {
        stmt.setNull(48, Types.CHAR);
      } else {
        stmt.setString(48, voGeneralBillHeader.getAttributeValue("vuserdef16").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef17") == null) {
        stmt.setNull(49, Types.CHAR);
      } else {
        stmt.setString(49, voGeneralBillHeader.getAttributeValue("vuserdef17").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef18") == null) {
        stmt.setNull(50, Types.CHAR);
      } else {
        stmt.setString(50, voGeneralBillHeader.getAttributeValue("vuserdef18").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef19") == null) {
        stmt.setNull(51, Types.CHAR);
      } else {
        stmt.setString(51, voGeneralBillHeader.getAttributeValue("vuserdef19").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("vuserdef20") == null) {
        stmt.setNull(52, Types.CHAR);
      } else {
        stmt.setString(52, voGeneralBillHeader.getAttributeValue("vuserdef20").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc1") == null) {
        stmt.setNull(53, Types.CHAR);
      } else {
        stmt.setString(53, voGeneralBillHeader.getAttributeValue("pk_defdoc1").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc2") == null) {
        stmt.setNull(54, Types.CHAR);
      } else {
        stmt.setString(54, voGeneralBillHeader.getAttributeValue("pk_defdoc2").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc3") == null) {
        stmt.setNull(55, Types.CHAR);
      } else {
        stmt.setString(55, voGeneralBillHeader.getAttributeValue("pk_defdoc3").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc4") == null) {
        stmt.setNull(56, Types.CHAR);
      } else {
        stmt.setString(56, voGeneralBillHeader.getAttributeValue("pk_defdoc4").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc5") == null) {
        stmt.setNull(57, Types.CHAR);
      } else {
        stmt.setString(57, voGeneralBillHeader.getAttributeValue("pk_defdoc5").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc6") == null) {
        stmt.setNull(58, Types.CHAR);
      } else {
        stmt.setString(58, voGeneralBillHeader.getAttributeValue("pk_defdoc6").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc7") == null) {
        stmt.setNull(59, Types.CHAR);
      } else {
        stmt.setString(59, voGeneralBillHeader.getAttributeValue("pk_defdoc7").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc8") == null) {
        stmt.setNull(60, Types.CHAR);
      } else {
        stmt.setString(60, voGeneralBillHeader.getAttributeValue("pk_defdoc8").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc9") == null) {
        stmt.setNull(61, Types.CHAR);
      } else {
        stmt.setString(61, voGeneralBillHeader.getAttributeValue("pk_defdoc9").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc10") == null) {
        stmt.setNull(62, Types.CHAR);
      } else {
        stmt.setString(62, voGeneralBillHeader.getAttributeValue("pk_defdoc10").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc11") == null) {
        stmt.setNull(63, Types.CHAR);
      } else {
        stmt.setString(63, voGeneralBillHeader.getAttributeValue("pk_defdoc11").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc12") == null) {
        stmt.setNull(64, Types.CHAR);
      } else {
        stmt.setString(64, voGeneralBillHeader.getAttributeValue("pk_defdoc12").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc13") == null) {
        stmt.setNull(65, Types.CHAR);
      } else {
        stmt.setString(65, voGeneralBillHeader.getAttributeValue("pk_defdoc13").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc14") == null) {
        stmt.setNull(66, Types.CHAR);
      } else {
        stmt.setString(66, voGeneralBillHeader.getAttributeValue("pk_defdoc14").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc15") == null) {
        stmt.setNull(67, Types.CHAR);
      } else {
        stmt.setString(67, voGeneralBillHeader.getAttributeValue("pk_defdoc15").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc16") == null) {
        stmt.setNull(68, Types.CHAR);
      } else {
        stmt.setString(68, voGeneralBillHeader.getAttributeValue("pk_defdoc16").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc17") == null) {
        stmt.setNull(69, Types.CHAR);
      } else {
        stmt.setString(69, voGeneralBillHeader.getAttributeValue("pk_defdoc17").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc18") == null) {
        stmt.setNull(70, Types.CHAR);
      } else {
        stmt.setString(70, voGeneralBillHeader.getAttributeValue("pk_defdoc18").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc19") == null) {
        stmt.setNull(71, Types.CHAR);
      } else {
        stmt.setString(71, voGeneralBillHeader.getAttributeValue("pk_defdoc19").toString());
      }
      if (voGeneralBillHeader.getAttributeValue("pk_defdoc20") == null) {
        stmt.setNull(72, Types.CHAR);
      } else {
        stmt.setString(72, voGeneralBillHeader.getAttributeValue("pk_defdoc20").toString());
      }
      
      // 供应商基本档案ID
      if (voGeneralBillHeader.getAttributeValue("pk_cubasdoc") == null) {
        stmt.setNull(73, Types.CHAR);
      } else {
        stmt.setString(73, voGeneralBillHeader.getAttributeValue("pk_cubasdoc").toString());
      }
      // 客户基本档案ID
      if (voGeneralBillHeader.getAttributeValue("pk_cubasdocC") == null) {
        stmt.setNull(74, Types.CHAR);
      } else {
        stmt.setString(74, voGeneralBillHeader.getAttributeValue("pk_cubasdocC").toString());
      }
      //add by zwx 更新盖子合计数 2016-1-27 
     
      if (voGeneralBillHeader.getGzhjs() == null)
          stmt.setNull(75, 3);
        else {
          stmt.setBigDecimal(75, voGeneralBillHeader.getGzhjs().toBigDecimal());
        }

        if (voGeneralBillHeader.getGzds() == null)  
          stmt.setNull(76, 3);
        else {
          stmt.setBigDecimal(76, voGeneralBillHeader.getGzds().toBigDecimal());
        }

        if (voGeneralBillHeader.getGzfys() == null)
          stmt.setNull(77, 3);
        else {
          stmt.setBigDecimal(77, voGeneralBillHeader.getGzfys().toBigDecimal());
        }

        if (voGeneralBillHeader.getGzkbs() == null)
          stmt.setNull(78, 3);
        else {
          stmt.setBigDecimal(78, voGeneralBillHeader.getGzkbs().toBigDecimal());
        }

        if (voGeneralBillHeader.getGghjs() == null)
          stmt.setNull(79, 3);
        else {
          stmt.setBigDecimal(79, voGeneralBillHeader.getGghjs().toBigDecimal());
        }

        if (voGeneralBillHeader.getGgds() == null)
          stmt.setNull(80, 3);
        else {
          stmt.setBigDecimal(80, voGeneralBillHeader.getGgds().toBigDecimal());
        }

        if (voGeneralBillHeader.getGgfys() == null)
          stmt.setNull(81, 3);
        else {
          stmt.setBigDecimal(81, voGeneralBillHeader.getGgfys().toBigDecimal());
        }

        if (voGeneralBillHeader.getGgkbs() == null)
          stmt.setNull(82, 3);
        else {
          stmt.setBigDecimal(82, voGeneralBillHeader.getGgkbs().toBigDecimal());
        }
      //end by zwx
      // find record by PK fields:
      stmt.setString(83, voGeneralBillHeader.getPrimaryKey());
...
public void updateItemBarcodeNumBatch(ArrayList voItems)
  ...
  GeneralBillItemVO voItem = null;
  String sql =
    "update ic_general_b set  nbarcodenum=?,bbarcodeclose=?  where cgeneralbid = ?";
...
//      barcodevo = voItem.getBarCodeVOs();
      ...
      stmt.setBigDecimal(1, voItem.getListbarcodenums().toBigDecimal());
  ...
  try {
    ...
    dmo = null;
      ...
      if(e instanceof SQLException)
          ...
          throw new SQLException(nc.bs.ml.NCLangResOnserver.getInstance().getStrByID("4008bill","UPP4008bill-000365")/*@res "更新表体错误！"*/);
  }
...
public void updateSourceRowNo(String sql) {
  ...
  try {
    con = getConnection();
  //String sql = " update ic_general_b set vsourcerowno=? where csourcebillbid=?";
 ...
 * 判断有无下游单据。
 * 仅用在取消签字的动作中。
 * 特殊之处：生成的存货核算单据不在下游单据之列。
 * 其他地方如有引用，请慎重！！
 * 
 * 邵兵 on Jun 8, 2005
 */
protected boolean isHaveForwardBill(String sCurBillType,String sCurBillPK)  throws Exception {
   
        //得到下游单据类型
      ArrayList alFwdBillType=SourceBillParaV3.getFwdBillTypes(sCurBillType);       
      
      if(alFwdBillType!=null){
            DefaultSourceDMO dmoFwd=new DefaultSourceDMO();
            ...
            for(int i=0;i<alFwdBillType.size();i++){
                ...
                if (sFwdBillType.charAt(0) == 'I' //存货核算单据不在下游单据之列
                  ||sFwdBillType.equals("4A")||sFwdBillType.equals("4I")){//库存哪些单据?拆解的下游可以
                    continue;
                }
                //应收单不再考虑，如果是4C
                if("4C".equals(sCurBillType) && (sFwdBillType.charAt(0) == 'D' || sFwdBillType.charAt(0) == 'd') )
                  continue;
                
              LightBillVO[] voaRet=dmoFwd.getForwardBills(sCurBillType, sCurBillPK, sFwdBillType);
              //有返回值表示有下游单据。
...
public ArrayList updateByid(GeneralBillItemVO[] vos) throws Exception{
  ...
  SmartDMO dmo=new SmartDMO();
  dmo.executeUpdateBatch(vos,new String[]{"vvehiclecode","ncountnum","vuserdef9"},new String[]{"cgeneralbid"});
  Object[] os=dmo.selectBy2("select cgeneralbid,ts from ic_general_b where 0=0 "+GeneralSqlString.formInSQL("cgeneralbid",alid));
  ArrayList alts=new ArrayList();
  if(os!=null&&os.length>0){
    alts=new ArrayList();
    for(int i=0;i<os.length;i++){
      
      alts.add(new String[]{(String)((Object[])os[i])[0],((Object[])os[i])[1].toString()});
